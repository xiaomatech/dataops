...
def setup_hadoop():
  ...
  """
    ...
    if params.has_namenode:
        ...
        if not WebHDFSUtil.is_webhdfs_available(params.is_webhdfs_enabled,
                                                params.dfs_type):
            ...
            if params.hadoop_metrics2_properties_content:
                File(
                    ...
                    content=InlineTemplate(
                        params.hadoop_metrics2_properties_content))
            else:
                File(
                    os.path.join(params.hadoop_conf_dir,
                                 "hadoop-metrics2.properties"),
...
def __setup_unlimited_key_jce_policy(custom_java_home, custom_jdk_name,
                                     custom_jce_name):
    """
  ...
  """
    ...
    if params.sysprep_skip_setup_jce:
        ...
        if jcePolicyInfo.is_unlimited_key_jce_policy():
            ...
            jce_zip_source = format(
                "{ambari_server_resources_url}/{custom_jce_name}")
            java_security_dir = format("{custom_java_home}/jre/lib/security")

            Logger.debug(
                "Downloading the unlimited key JCE policy files from {0} to {1}."
                    .format(jce_zip_source, jce_zip_target))
            ...
            Logger.debug("Removing existing JCE policy JAR files: {0}.".format(
                java_security_dir))
            File(
                format("{java_security_dir}/US_export_policy.jar"),
                action="delete")
            File(
                format("{java_security_dir}/local_policy.jar"),
                action="delete")
