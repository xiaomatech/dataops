...
stack_supports_hadoop_custom_extensions = True
...
jce_policy_zip = default("/ambariLevelParams/jce_name",
...
if has_metric_collector:
    ...
    metric_legacy_hadoop_sink = False
...
if (('hdfs-log4j' in config['configurations'])
        and ('content' in config['configurations']['hdfs-log4j'])):
    log4j_props = config['configurations']['hdfs-log4j']['content']
    if (('yarn-log4j' in config['configurations'])
            and ('content' in config['configurations']['yarn-log4j'])):
...
if command_params is not None:
    refresh_topology = bool(
        command_params["refresh_topology"]
    ) if "refresh_topology" in command_params else False
...
#topology files
net_topology_script_file_path = "/etc/hadoop/topology_script.py"
