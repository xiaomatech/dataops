...
DEFAULT_HADOOP_HDFS_EXTENSION_DIR = "/ext/hadoop"
DEFAULT_HADOOP_HIVE_EXTENSION_DIR = "/ext/hive"
DEFAULT_HADOOP_HBASE_EXTENSION_DIR = "/ext/hbase"


def setup_extensions():
    """
  The goal of this method is to distribute extensions (for example jar files) from
  HDFS (/ext/{major_stack_version}/{service_name}) to all nodes which contain related
  ...
  """
    ...
    hadoop_custom_extensions_hdfs_dir = get_config_formatted_value(
        default("/configurations/core-site/hadoop.custom-extensions.root",
                DEFAULT_HADOOP_HDFS_EXTENSION_DIR))
    ...
    hadoop_custom_extensions_local_dir = "{0}/ext/hadoop".format(
...
def setup_extensions_hive():
    ...
    hive_custom_extensions_owner = default(
        ...
        params.hdfs_user)
    hive_custom_extensions_hdfs_dir = DEFAULT_HADOOP_HIVE_EXTENSION_DIR

    hive_custom_extensions_local_dir = "{0}/ext/hive".format(
        Script.get_stack_root())
...
def download_extensions(owner_user, owner_group, hdfs_source_dir,
  ...
  """
    ...
    if not os.path.isdir(local_target_dir):
        ...
        only_if_cmd = "ls -d {extensions_tmp_dir}/*".format(
            extensions_tmp_dir=extensions_tmp_dir)
...
def clean_extensions(local_dir):
  ...
  """
    if os.path.isdir(local_dir):
        Directory(local_dir, action="delete")
    return True
