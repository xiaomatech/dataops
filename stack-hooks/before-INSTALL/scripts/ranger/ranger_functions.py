...
class Rangeradmin:
    ...
    """
        try:
            ...
            if isinstance(e, urllib2.HTTPError):
                ...
                raise Fail(
                    "Connection to Ranger Admin failed. Reason - {0}.".format(
                        e.reason))
    ...
    @safe_retry(
        ...
        err_class=Fail,
        return_on_fail=None)
    def get_policy_by_repo_name(self, name, component, status,
                                usernamepassword):
        """
    ...
    """
        try:
            ...
            if isinstance(e, urllib2.HTTPError):
                ...
                raise Fail(
                    "Error getting policy from repository {0} for component {1}. Reason - {2}."
                    .format(name, component, e.reason))
        except httplib.BadStatusLine:
            raise Fail(
                "Ranger Admin service is not reachable, please restart the service and then try again"
    ...
    """
        ...
        if typeOfPolicy == "hdfs":
            ...
            policyObj['permMapList'] = [{
                'userList': [policy_user],
                'permList': [
                    'select', 'update', 'create', 'drop', 'alter', 'index',
                    'lock', 'all', 'admin'
                ]
            }]
            ...
            policyObj['permMapList'] = [{
                'userList': [policy_user],
                'permList': ['allow', 'admin']
            }]
        elif typeOfPolicy == "storm":
            policyObj['permMapList'] = [{
                'userList': [policy_user],
                'permList': [
                    'submitTopology', 'fileUpload', 'getNimbusConf',
                    'getClusterInfo', 'fileDownload', 'killTopology',
                    'rebalance', 'activate', 'deactivate', 'getTopologyConf',
                    'getTopology', 'getUserTopology', 'getTopologyInfo',
                    'uploadNewCredential', 'admin'
                ]
            }]
        return policyObj
