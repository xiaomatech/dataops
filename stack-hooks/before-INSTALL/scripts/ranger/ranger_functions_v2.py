...
class RangeradminV2:
    ...
    def create_ranger_repository(self,
        ...
        if not is_stack_supports_ranger_kerberos or not is_security_enabled:
            ...
            response = self.check_ranger_login_curl(
                ...
                component_user_principal, self.url_login, True)
    ...
    """
        try:
            ...
            if isinstance(e, urllib2.HTTPError):
                raise Fail(
                    "Connection failed to Ranger Admin. Http status code - {0}. \n {1}"
                    .format(e.code, e.read()))
            else:
                raise Fail(
                    "Connection failed to Ranger Admin. Reason - {0}.".format(
                        e.reason))
    ...
    @safe_retry(
        ...
        err_class=Fail,
        return_on_fail=None)
    def check_ranger_login_curl(self, component_user, component_user_keytab,
                                component_user_principal, base_url, True):
        """
    :param url: ranger admin host url
    :param usernamepassword: user credentials using which repository needs to be searched.
    :return: Returns login check response
    ...
    """
        try:
            ...
            if response_stripped and len(response_stripped) > 0:
                ...
                if 'name' in response_json and response_json['name'].lower(
                ) == name.lower():
            ...
            raise Fail(
                'Error in call for getting Ranger service:\n {0}'.format(err))
    ...
    @safe_retry(
        ...
        err_class=Fail,
        return_on_fail=None)
    def create_repository_curl(self,
                               component_user,
                               component_user_keytab,
                               component_user_principal,
                               name,
    ...
    """
        try:
            ...
            response, error_message, time_in_millis = self.call_curl_request(
                component_user, component_user_keytab,
                component_user_principal, search_repo_url, False, method, data,
                header)
            if response and len(response) > 0:
                ...
                if 'name' in response_json and response_json['name'].lower(
                    ...
                    Logger.info(
                        'Repository {name} already exists'.format(name=name))
                    return response_json
                else:
            ...
            raise Fail(
                'Error in call for creating Ranger service:\n {0}'.format(err))
    ...
    """
        try:
            ...
            raise Fail(
                "Ranger Admin is not reachable for updating service name {0} for service {1}"
                .format(repo_name, component))
        except TimeoutError:
            raise Fail("Connection to Ranger Admin failed. Reason - timeout")
    ...
    @safe_retry(
        ...
        err_class=Fail,
        return_on_fail=None)
    def update_repository_curl(self,
                               component,
                               repo_name,
                               repo_properties,
                               component_user,
    ...
    """
        try:
            ...
            response, error_message, time_in_millis = self.call_curl_request(
                component_user, component_user_keytab,
                component_user_principal, update_repo_url, False, method,
                repo_update_data, header)
            if response and len(response) > 0:
                response_json = json.loads(response)
            ...
            raise Fail(
                'Error updating service name {0} on Ranger Admin for service {1}.\n Reason - {2}'
                .format(repo_name, component, err))
