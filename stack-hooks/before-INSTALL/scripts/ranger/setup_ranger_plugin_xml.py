...
"""
__all__ = [
    "setup_ranger_plugin", "get_audit_configs",
...
def setup_ranger_plugin(component_select_name,
    ...
    if plugin_enabled:
        ...
        setup_ranger_plugin_keystore(
            service_name, audit_db_is_enabled, credential_file,
...
def setup_ranger_plugin_jar_symblink(service_name, component_list):
    stack_root = Script.get_stack_root().replace('/usr/hdp','/opt')
    jar_files = os.listdir(
        format('{stack_root}/ranger-{service_name}-plugin/lib'))
    ...
    for jar_file in jar_files:
        for component in component_list:
            Execute(
                ('ln', '-sf',
                 format(
                     '{stack_root}/ranger-{service_name}-plugin/lib/{jar_file}'
                 ), format('{stack_root}/{component}/lib/{jar_file}')),
                not_if=format('ls {stack_root}/{component}/lib/{jar_file}'),
                only_if=format(
                    'ls {stack_root}/ranger-{service_name}-plugin/lib/{jar_file}'
                ),
                sudo=True)
...
def get_audit_configs(config):
    ...
    if xa_audit_db_flavor == 'mysql':
        jdbc_jar_name = default("/ambariLevelParams/custom_mysql_jdbc_name",
                                None)
        ...
        jdbc_jar_name = default("/ambariLevelParams/custom_oracle_jdbc_name",
                                None)
        previous_jdbc_jar_name = default(
        ...
        audit_jdbc_url = format(
            'jdbc:postgresql://{xa_db_host}/{xa_audit_db_name}')
        jdbc_driver = "org.postgresql.Driver"
    elif xa_audit_db_flavor == 'mssql':
        jdbc_jar_name = default("/ambariLevelParams/custom_mssql_jdbc_name",
                                None)
        previous_jdbc_jar_name = default(
