...
class DefaultStackAdvisor(StackAdvisor):
    ...
    def initialize_logger(self, name='DefaultStackAdvisor', logging_level=logging.INFO,
            ...
            try:
                with open(path, 'rb') as fp:
                    ...
                    if hasattr(service_advisor, best_class_name):
                        self.logger.info(
                            "ServiceAdvisor implementation for service {0} was loaded".format(service_name))
                ...
                self.logger.exception(
                    "Failed to load or create ServiceAdvisor implementation for service {0}".format(service_name))
    ...
    def createComponentLayoutRecommendations(self, services, hosts):
        ...
        for service in sortedServices:
            ...
            for component in masterComponents:
                ...
                filteredHosts = self.getFilteredHostsBasedOnDependencies(services, component, hostsList,
                                                                         hostsComponentsMap)
        ...
        usedHostsListList = [component["StackServiceComponents"]["hostnames"] for component in componentsList if
                             not self.isComponentNotValuable(component)]
        ...
        for service in sortedServices:
            ...
            for component in slaveClientComponents:
                ...
                hostsForComponent = advisor.getHostsForSlaveComponent(services, hosts, component, filteredHosts,
                                                                      filteredFreeHosts)
    ...
    def sortServicesByDependencies(self, services, service, processedServices, sortedServices):
        ...
        for component in components:
            ...
            for dependency in dependencies:
                ...
                conditionsPresent = "conditions" in dependency["Dependencies"] and dependency["Dependencies"][
                    "conditions"]
        ...
        for dependency in dependencies:
            ...
            if not conditionsPresent:
                ...
                if (requiredComponent is not None) and (requiredComponent["component_category"] != "CLIENT"):
                    scope = "cluster" if "scope" not in dependency["Dependencies"] else dependency["Dependencies"][
                        "scope"]
                    if scope == "host":
                ...
                if (self.isSlaveComponent(component) or self.isMasterComponent(component)) and \
                    ...
                    for dependency in component['dependencies']:
                        ...
                        if not conditionsPresent:
                            ...
                            requiredComponentHosts = requiredComponent[
                                "hostnames"] if requiredComponent is not None else []
                            ...
                            if (requiredComponent is None) or \
                                ...
                                if scope == "host":
                                    ...
                                    if requiredComponentHostsAbsent:
                                        message = "{0} requires {1} to be co-hosted on following host(s): {2}.".format(
                                    ...
                                    message = "{0} requires {1} to be present in the cluster.".format(
                                        componentDisplayName, requiredComponentDisplayName)
                                    items.append({"type": 'host-component', "level": 'ERROR', "message": message,
                                                  "component-name": component["StackServiceComponents"][
                                                      "component_name"]})
        return items
    ...
    def calculateYarnAllocationSizes(self, configurations, services, hosts):
        ...
        putYarnProperty('yarn.scheduler.minimum-allocation-mb', 1024)
        putYarnProperty('yarn.scheduler.maximum-allocation-mb', 1024000)
    ...
    def getConfigurationClusterSummary(self, servicesList, hosts, components, services):
        ...
        if services:  # its never None but some unit tests pass it as None
            ...
            if self.getOldValue(services, "yarn-site", "yarn.scheduler.minimum-allocation-mb") or \
                ...
                if "yarn-site" in services["configurations"] and \
                        "yarn.scheduler.minimum-allocation-mb" in services["configurations"]["yarn-site"][
                    "properties"] and \
                        str(services["configurations"]["yarn-site"]["properties"][
                                "yarn.scheduler.minimum-allocation-mb"]).isdigit():
                    ...
                    self.logger.info(
                        "Minimum ram per container due to user input - cluster[yarnMinContainerSize]: " + str(
                    ...
                    if cluster["yarnMinContainerSize"] > cluster["totalAvailableRam"]:
                        ...
                        self.logger.info(
                            "Minimum ram per container after checking against limit - cluster[yarnMinContainerSize]: " + str(
                                cluster["yarnMinContainerSize"]))
                        pass
                    cluster["minContainerSize"] = cluster[
                        "yarnMinContainerSize"]  # set to what user has suggested as YARN min container size
        ...
        self.logger.info(
            "Ram per containers before normalization - cluster[ramPerContainer]: " + str(cluster["ramPerContainer"]))
        ...
        if cluster["ramPerContainer"] > cluster["yarnMinContainerSize"]:
            cluster["ramPerContainer"] = int(cluster["ramPerContainer"] / cluster["yarnMinContainerSize"]) * cluster[
                    ...
                    if propertyName in configurations[configName]["properties"]:
                        if "maximum" in recommendedDefaults[configName]["property_attributes"][propertyName] and \
                            ...
                            maxValue = convertToNumber(
                                recommendedDefaults[configName]["property_attributes"][propertyName]["maximum"])
                            if userValue > maxValue:
                                validationItems.extend([{"config-name": propertyName, "item": self.getWarnItem(
                                    "Value is greater than the recommended maximum of {0} ".format(maxValue))}])
                        if "minimum" in recommendedDefaults[configName]["property_attributes"][propertyName] and \
                                propertyName in recommendedDefaults[configName]["properties"]:
                            userValue = convertToNumber(configurations[configName]["properties"][propertyName])
                            minValue = convertToNumber(
                                recommendedDefaults[configName]["property_attributes"][propertyName]["minimum"])
                            if userValue < minValue:
                                validationItems.extend([{"config-name": propertyName, "item": self.getWarnItem(
                                    "Value is less than the recommended minimum of {0} ".format(minValue))}])
    ...
    def getConfigurationsValidationItemsForService(self, configurations, recommendedDefaults, service, services, hosts):
        ...
        if validator is not None:
            for siteName, method in validator.items():
                resultItems = self.validateConfigurationsForSite(configurations, recommendedDefaults, services, hosts,
                                                                 siteName, method)
        ...
        if serviceAdvisor is not None:
            items.extend(
                serviceAdvisor.getServiceConfigurationsValidationItems(configurations, recommendedDefaults, services,
                                                                       hosts))
    ...
    def recommendConfigGroupsConfigurations(self, recommendations, services, components, hosts,
        ...
        for configGroup in services["config-groups"]:
            ...
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendations(configurations, cgClusterSummary, cgServices,
                                                                      cgHosts)
            ...
            for service in services["services"]:
                ...
                if calculation is not None:
                    ...
                    if serviceAdvisor is not None:
                        serviceAdvisors.append(serviceAdvisor)
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendationsForSSO(configurations, clusterSummary, services,
                                                                            hosts)
            ...
            for service in services["services"]:
                ...
                if calculation is not None:
                    ...
                    if serviceAdvisor is not None:
                        serviceAdvisors.append(serviceAdvisor)
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendationsForKerberos(configurations, clusterSummary,
                                                                                 services, hosts)
    ...
    def isHostSuitableForComponent(self, host, component):
        return not (self.getComponentName(component) in self.getNotPreferableOnServerComponents() and self.isLocalHost(
            host))
    ...
    def getServiceHadoopProxyUsersConfigurationDict(self):
        ...
        return {
            ...
            "HIVE": [("hive-env", "hive_user",
                      {HOSTS_PROPERTY: ["HIVE_SERVER", "HIVE_SERVER_INTERACTIVE"], GROUPS_PROPERTY: ALL_WILDCARD}),
                     ("hive-env", "webhcat_user", {HOSTS_PROPERTY: ["WEBHCAT_SERVER"], GROUPS_PROPERTY: ALL_WILDCARD})],
            "YARN": [("yarn-env", "yarn_user", {HOSTS_PROPERTY: ["RESOURCEMANAGER"]}, lambda services, hosts: len(
                self.getHostsWithComponent("YARN", "RESOURCEMANAGER", services, hosts)) > 1)],
            ...
            for user, (userNameConfig, userNameProperty, hostSelectorMap) in usersComponents.iteritems():
                ...
                for proxyPropertyName, hostSelector in hostSelectorMap.iteritems():
                    ...
                    if isinstance(hostSelector, (list, tuple)):
                        _, componentHostNames = self.get_data_for_proxyuser(user, services,
                                                                            configurations)  # preserve old values
                        ...
                        self.logger.info(
                            "Host List for [service='{0}'; user='{1}'; components='{2}']: {3}".format(serviceName, user,
                                                                                                      ','.join(
                                                                                                          hostSelector),
    ...
    def recommendHadoopProxyUsers(self, configurations, services, hosts):
        ...
        if "HIVE" in servicesList:
            hive_user = get_from_dict(services, ("configurations", "hive-env", "properties", "hive_user"),
                                      default_value=None)
        ...
        for user_name, user_properties in users.iteritems():
            ...
            self.logger.info(
                "Updated hadoop.proxyuser.{0}.hosts as : {1}".format(user_name, user_properties["propertyHosts"]))
            if "propertyGroups" in user_properties:
    ...
    def validatorEnoughDiskSpace(self, properties, propertyName, hostInfo, reqiuredDiskSpace):
        ...
        if mountPoint is None:
            return self.getErrorItem(
                "No mount point in directory %s. Mount points: %s" % (dir, ', '.join(mountPoints.keys())))
    ...
    def isConfigPropertiesChanged(self, services, config_type, config_names, all_exists=True):
        ...
        changed_config_names_set = set(
            [changedConfig['name'] for changedConfig in changedConfigs if changedConfig['type'] == config_type])
        ...
        try:
            ...
            if valueInt < defaultValueInt:
                return self.getWarnItem(
                    "Value is less than the minimum recommended default of -Xmx" + str(defaultValue))
        except:
            return None
