...
class TezServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def getServiceConfigurationRecommendations(self, configurations,
        ...
        recommender.recommendTezConfigurationsFromHDP23(
            configurations, clusterData, services, hosts)
        recommender.recommendTezConfigurationsFromHDP26(
            configurations, clusterData, services, hosts)
...
class TezRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendTezConfigurationsFromHDP21(self, configurations, clusterData,
        ...
        putTezProperty("tez.am.resource.memory.mb", 30720)
        putTezProperty(
            "tez.am.java.opts",
    ...
    def recommendTezConfigurationsFromHDP22(self, configurations, clusterData,
        ...
        putTezProperty(
            ...
            min(30720,
                ...
                if int(clusterData['amMemory']) < 3072 else int(
                    clusterData['amMemory'])))

        taskResourceMemory = clusterData[
        ...
        taskResourceMemory = min(
            ...
            taskResourceMemory, yarnMaxAllocationSize)
        putTezProperty(
            ...
            min(
                int(configurations["yarn-site"]["properties"]
                    ["yarn.scheduler.maximum-allocation-mb"]),
                taskResourceMemory))
        taskResourceMemory = int(configurations["tez-site"]["properties"]
                                 ["tez.task.resource.memory.mb"])
        putTezProperty("tez.runtime.io.sort.mb",
                       min(int(taskResourceMemory * 0.4), 2047))
        ...
        tez_queue = self.recommendYarnQueue(services, "tez-site",
                                            "tez.queue.name")
        ...
        if serverProperties:
            ...
            if os.path.exists(views_work_dir) and os.path.isdir(
                    views_work_dir):
        ...
        if has_tez_view:
            tez_url = '{0}://{1}:{2}/#/main/view/TEZ/tez_cluster_instance'.format(
    ...
    def recommendTezConfigurationsFromHDP23(self, configurations, clusterData,
        ...
        if "HIVE" in self.getServiceNames(services):
            ...
            if "hive-site" in configurations and "hive.tez.container.size" in configurations[
                "hive-site"]["properties"]:
                putTezProperty(
                    "tez.task.resource.memory.mb", configurations["hive-site"]
        ...
        putTezProperty("tez.runtime.io.sort.mb",
                       int(taskResourceMemory * 0.8 * 0.33))

        if "tez-site" in services["configurations"] and "tez.runtime.sorter.class" in \
                services["configurations"]["tez-site"]["properties"]:
            if services["configurations"]["tez-site"]["properties"][
                "tez.runtime.sorter.class"] == "LEGACY":
                putTezAttribute = self.putPropertyAttribute(
                    configurations, "tez-site")
        ...
        if "ambari-server-properties" in services and "java.home" in services[
            "ambari-server-properties"]:
            # JDK8 needs different parameters
            jdkMajorVersion = self.__getJdkMajorVersion(
                services["ambari-server-properties"]["java.home"])
            if jdkMajorVersion and len(jdkMajorVersion) > 1 and int(
                    jdkMajorVersion[0]) > 0 and int(jdkMajorVersion[1]) > 7:
    ...
    def recommendTezConfigurationsFromHDP26(self, configurations, clusterData,
                                            services, hosts):
        putTezProperty = self.putProperty(configurations, "tez-site")
        ...
        self.logger.info(
            "Updated 'tez-site' config 'tez.task.launch.cmd-opts' and 'tez.am.launch.cmd-opts' as "
            ": {0}".format(tez_jvm_updated_opts))

    def recommendTezConfigurationsFromHDP30(self, configurations, clusterData,
                                            services, hosts):
        putTezProperty = self.putProperty(configurations, "tez-site")
        if "HIVE" in self.getServiceNames(services) and "hive-site" in services[
            "configurations"] and "hive.metastore.warehouse.external.dir" in services[
            "configurations"]["hive-site"]["properties"]:
            hive_metastore_warehouse_external_dir = services["configurations"][
                "hive-site"]["properties"][
    ...
    def __getJdkMajorVersion(self, javaHome):
        jdkVersion = subprocess.check_output(
            [javaHome + "/bin/java", "-version"], stderr=subprocess.STDOUT)
        pattern = '\"(\d+\.\d+).*\"'
...
class TezValidator(service_advisor.ServiceAdvisor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [("tez-site",
                            self.validateTezConfigurationsFromHDP21),
                           ("tez-site",
                            self.validateTezConfigurationsFromHDP22)]
        ...
        validationItems = [{
        ...
        },
            {
                "config-name":
                    'tez.am.java.opts',
                "item":
                    self.validateXmxValue(properties,
                                          recommendedDefaults,
                                          'tez.am.java.opts')
            },
            {
    ...
    def validateTezConfigurationsFromHDP22(self, properties,
        ...
        yarnSiteProperties = self.getSiteProperties(configurations,
                                                    "yarn-site")
        if yarnSiteProperties:
            ...
            if int(tez_site[prop_name1]) > yarnMaxAllocationSize:
                validationItems.append({
                    ...
                    "item":
                        self.getWarnItem(
                            "{0} should be less than YARN max allocation size ({1})"
                                .format(prop_name1, yarnMaxAllocationSize))
                })
            if int(tez_site[prop_name2]) > yarnMaxAllocationSize:
                validationItems.append({
                    ...
                    "item":
                        self.getWarnItem(
                            "{0} should be less than YARN max allocation size ({1})"
                                .format(prop_name2, yarnMaxAllocationSize))
                })
