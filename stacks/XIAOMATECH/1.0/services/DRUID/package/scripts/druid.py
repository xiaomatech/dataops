...
def install_extension():
    import params
    extensions_dir = params.install_dir + '/extensions/'
    extension_jar_files_conf = default(
        "/configurations/druid-env/extensions_download", '')
    if extension_jar_files_conf != '':
        share_jar_files = extension_jar_files_conf.split(',')
        for jar_file in share_jar_files:
            jar_file_path = extensions_dir + jar_file.replace(
                '.tar.gz', '').replace('.tgz', '')
            if not os.path.exists(jar_file_path):
                Execute(
                    'wget ' + download_url_base + '/druid/' + jar_file +
                    ' -O /tmp/' + jar_file,
                    user=params.druid_user)
                Execute(
                    'tar -zxvf /tmp/' + jar_file + ' -C ' + extensions_dir,
                    user=params.druid_user)
...
def install_druid():
    ...
    if not os.path.exists(Script.get_stack_root() + '/' + params.version_dir) or not os.path.exists(
        ...
        Execute('chown -R %s:%s %s/%s' %
                (params.druid_user, params.user_group,params.stack_root, params.version_dir))
        Execute('chown -R %s:%s %s' % (params.druid_user, params.user_group,
                                       params.install_dir))
...
def druid(upgrade_type=None, nodeType=None):
    ...
    File(
        format("{params.druid_common_conf_dir}/druid-log4j.xml"),
        mode=0644,
        owner=params.druid_user,
        group=params.user_group,
    ...
    for node_type in [
        ...
        if ['middleManager', 'overlord'].__contains__(node_type_lowercase):
            try:
                Logger.info(
                    format(
                ...
                Execute(
                    format(
                        '{sudo} cp /usr/share/java/hadoop/hadoop-gpl-compression*.jar {druid_hadoop_dependencies_dir}/hadoop-client/*/'
                    ))
            except Fail as ex:
                Logger.info(
                    format(
                        "No Hadoop LZO found at {hadoop_lib_home}/hadoop-gpl-compression*.jar"
                    ))
    generate_logfeeder_input_config(
        'druid', Template(
            "input.config-druid.json.j2", extra_imports=[default]))
...
def create_hadoop_directory(hadoop_dir, mode=0755):
    ...
    params.HdfsResource(
        ...
        mode=mode)
    Logger.info(
        format("Created Hadoop Directory [{hadoop_dir}], with mode [{mode}]"))
...
def ensure_base_directories():
    ...
    Directory(
        [
            ...
            params.druid_middlemanager_conf_dir,
            params.druid_historical_conf_dir, params.druid_overlord_conf_dir,
            params.druid_router_conf_dir, params.druid_segment_infoDir,
