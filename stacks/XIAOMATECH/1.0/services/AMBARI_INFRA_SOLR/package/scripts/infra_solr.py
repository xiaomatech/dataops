...
class InfraSolr(Script):
    ...
    def start(self, env):
        ...
        generate_logfeeder_input_config(
            'ambari-infra',
            Template(
                "input.config-ambari-infra.json.j2", extra_imports=[default]))

        setup_solr_znode_env()
        start_cmd = format(
            '{solr_bindir}/solr start -cloud -noprompt -s {infra_solr_datadir} -z {zookeeper_quorum}{infra_solr_znode} -p {infra_solr_port} -Dsolr.kerberos.name.rules=\'{infra_solr_kerberos_name_rules}\' 2>&1') \
            if params.security_enabled else format(
            '{solr_bindir}/solr start -cloud -noprompt -s {infra_solr_datadir} -z {zookeeper_quorum}{infra_solr_znode} -p {infra_solr_port} 2>&1')

        check_process = format("{sudo} test -f {infra_solr_pidfile} && {sudo} pgrep -F {infra_solr_pidfile}")

        piped_start_cmd = format(
            'source {infra_solr_conf}/infra-solr-env.sh; {start_cmd} | tee {infra_solr_log}') + '; (exit "${PIPESTATUS[0]}")'
        Execute(
            piped_start_cmd,
            environment={
        ...
        Execute(
            "echo `ps aux|grep 'ambari-infra-solr' |grep " + params.infra_solr_port + "| grep -v grep | awk '{print $2}'` > " + params.infra_solr_pidfile)
    ...
    def stop(self, env, upgrade_type=None):
        ...
        try:
            ...
            piped_stop_cmd = format('{stop_cmd} | tee {infra_solr_log}'
                                    ) + '; (exit "${PIPESTATUS[0]}")'
            Execute(
                piped_stop_cmd,
                environment={
    ...
    def kill_process(self, pid_file, user, log_dir):
        ...
        pid = get_user_call_output(
            format("cat {pid_file}"), user=user, is_checked_call=False)[1]
