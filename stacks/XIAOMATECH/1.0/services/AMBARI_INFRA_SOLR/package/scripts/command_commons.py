...
request_time_interval = int(
    default("/commandParams/solr_request_time_interval", 5))
...
solr_restore_config_set = default("/commandParams/solr_restore_config_set",
                                  None)
...
if solr_hdfs_path:
    ...
    hadoop_bin = params.stack_root + '/hadoop/sbin/'
    hadoop_bin_dir = params.stack_root + '/hadoop/bin/'
    hadoop_conf_dir = '/etc/hadoop'
    hadoop_conf_secure_dir = os.path.join(hadoop_conf_dir, "secure")
    hadoop_lib_home = params.stack_root + '/hadoop/lib/'
    hdfs_principal_name = default(
        '/configurations/hadoop-env/hdfs_principal_name', None)
    hdfs_user_keytab = params.config['configurations']['hadoop-env'][
        'hdfs_user_keytab']
...
def create_solr_api_request_command(request_path,
                                    output=None,
                                    override_solr_base_url=None):
    solr_url = format("{solr_base_url}/{request_path}"
                      ) if override_solr_base_url is None else format(
                          "{override_solr_base_url}/{request_path}")
...
def snapshot_status_check(request_cmd,
                          json_output,
                          snapshot_name,
                          backup=True,
                          log_output=True,
                ...
                if backup:
                    ...
                    if 'backup' in details:
                        ...
                        if type(backup_list) == type(
                                list()):  # support map and list format as well
                            backup_data = dict(
                                backup_list[i:i + 2]
                        ...
                        if (not 'snapshotName' in backup_data
                            ) or backup_data['snapshotName'] != snapshot_name:
                    ...
                    if 'restorestatus' in json_data:
                        ...
                        if (not 'snapshotName' in restorestatus_data
                        ...
                        if restorestatus_data['status'] == 'success':
                            Logger.info(
                                "Restore command successfully finished.")
                            failed = False
...
def resolve_ip_to_hostname(ip):
    try:
        ...
        fqdn_name = socket.getaddrinfo(host_name, 0, 0, 0, 0,
                                       socket.AI_CANONNAME)[0][3].lower()
...
def create_command(command):
    ...
    kinit_cmd = "{0} -kt {1} {2};".format(
        kinit_path_local, params.infra_solr_kerberos_keytab, params.
        infra_solr_kerberos_principal) if params.security_enabled else ""
...
def move_hdfs_folder(source_dir, target_dir):
    cmd = create_command(format(hadoop_bin_dir + '/hdfs dfs -mv {source_dir} {target_dir}'))
...
def check_folder_until_size_not_changes(dir):
    ...
    while size_changed:
        returncode, stdout = call(
            cmd, user=params.infra_solr_user, timeout=300)
        if stdout:
