...
class AtlasServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    """
        ...
        recommender.recommendAtlasConfigurationsFromHDP25(
            configurations, clusterData, services, hosts)
        recommender.recommendAtlasConfigurationsFromHDP26(
            configurations, clusterData, services, hosts)
    ...
    def isKerberosEnabled(services, configurations):
      ...
      """
        if configurations and "application-properties" in configurations and \
            ...
            return configurations["application-properties"]["properties"][
                "atlas.authentication.method.kerberos"].lower() == "true"
            ...
            return services["configurations"]["application-properties"][
                "properties"]["atlas.authentication.method.kerberos"].lower(
                ) == "true"
        else:
            return False
...
class AtlasRecommender(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    """
        ...
        atlas_server_hosts_info = self.getHostsWithComponent(
            "ATLAS", "ATLAS_SERVER", services, hosts)
        if is_atlas_in_cluster and atlas_server_hosts_info and len(
                atlas_server_hosts_info) > 0:
            # Multiple Atlas Servers can exist, so sort by hostname to create deterministic csv
            atlas_host_names = [
                e['Hosts']['host_name'] for e in atlas_server_hosts_info
            ]
            ...
            if 'application-properties' in services['configurations']:
                ...
                if 'atlas.server.http.port' in services['configurations'][
                        'application-properties']['properties']:
                    metadata_port = str(
                        services['configurations']['application-properties']
            ...
            atlas_rest_address_list = [
                "{0}://{1}:{2}".format(scheme, hostname, metadata_port)
                for hostname in atlas_host_names
            ]
            atlas_rest_address = ",".join(atlas_rest_address_list)
            self.logger.info(
                "Constructing atlas.rest.address=%s" % atlas_rest_address)
        return atlas_rest_address

    def recommendAtlasConfigurationsFromHDP25(self, configurations,
                                              clusterData, services, hosts):
        putAtlasApplicationProperty = self.putProperty(
            configurations, "application-properties", services)
        putAtlasRangerPluginProperty = self.putProperty(
            configurations, "ranger-atlas-plugin-properties", services)
        putAtlasEnvProperty = self.putProperty(configurations, "atlas-env",
                                               services)
        ...
        if atlas_rest_address is not None:
            putAtlasApplicationProperty("atlas.rest.address",
                                        atlas_rest_address)

        if "AMBARI_INFRA_SOLR" in servicesList and 'infra-solr-env' in services[
                'configurations']:
            if 'infra_solr_znode' in services['configurations'][
                    'infra-solr-env']['properties']:
        ...
        if "CONFLUENT" in servicesList:
            ...
            security_enabled = AtlasServiceAdvisor.isKerberosEnabled(
                services, configurations)
            ...
            putAtlasApplicationProperty('atlas.kafka.zookeeper.connect',
                                        zookeeper_host_port + '/kafka')
        else:
            putAtlasApplicationProperty('atlas.kafka.bootstrap.servers', "")
            putAtlasApplicationProperty('atlas.kafka.zookeeper.connect', "")

        if "HBASE" in servicesList and 'hbase-site' in services[
                'configurations']:
            if 'hbase.zookeeper.quorum' in services['configurations'][
                    'hbase-site']['properties']:
                hbase_zookeeper_quorum = services['configurations'][
                    'hbase-site']['properties']['hbase.zookeeper.quorum']
            else:
        ...
        if "ranger-env" in services["configurations"] and "ranger-atlas-plugin-properties" in services["configurations"] and \
            ...
            ranger_atlas_plugin_enabled = services["configurations"][
                "ranger-env"]["properties"]["ranger-atlas-plugin-enabled"]
            putAtlasRangerPluginProperty('ranger-atlas-plugin-enabled',
                                         ranger_atlas_plugin_enabled)

        ranger_atlas_plugin_enabled = ''
        if 'ranger-atlas-plugin-properties' in configurations and 'ranger-atlas-plugin-enabled' in configurations[
                'ranger-atlas-plugin-properties']['properties']:
            ranger_atlas_plugin_enabled = configurations[
                'ranger-atlas-plugin-properties']['properties'][
                    'ranger-atlas-plugin-enabled']
        elif 'ranger-atlas-plugin-properties' in services[
                'configurations'] and 'ranger-atlas-plugin-enabled' in services[
                    'configurations']['ranger-atlas-plugin-properties'][
                        'properties']:
            ranger_atlas_plugin_enabled = services['configurations'][
                'ranger-atlas-plugin-properties']['properties'][
                    'ranger-atlas-plugin-enabled']

        if ranger_atlas_plugin_enabled and (
                ranger_atlas_plugin_enabled.lower() == 'Yes'.lower()):
        ...
        if 'atlas-env' in services['configurations']:
            ...
            if 'atlas_server_metadata_size' in services['configurations'][
                    'atlas-env']['properties']:
                atlas_server_metadata_size = float(
                    services['configurations']['atlas-env']['properties']
                    ['atlas_server_metadata_size'])
            ...
            putAtlasEnvProperty("atlas_server_max_new_size",
                                atlas_server_max_new_size)

    def recommendAtlasConfigurationsFromHDP26(self, configurations,
                                              clusterData, services, hosts):
        servicesList = [
            service["StackServices"]["service_name"]
            for service in services["services"]
...
class AtlasValidator(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [("application-properties",
                            self.validateAtlasConfigurationsFromHDP25),
                           ("atlas-env",
                            self.validateAtlasConfigurationsFromHDP30)]

    def validateAtlasConfigurationsFromHDP25(self, properties,
                                             recommendedDefaults,
                                             configurations, services, hosts):
        application_properties = self.getSiteProperties(
            configurations, "application-properties")
        validationItems = []

        auth_type = application_properties[
            'atlas.authentication.method.ldap.type']
        auth_ldap_enable = application_properties[
            'atlas.authentication.method.ldap'].lower() == 'true'
        ...
        if auth_ldap_enable:
            for prop in props_to_require:
                if prop not in application_properties or application_properties[
                        prop] is None or application_properties[prop].strip(
                        ) == "":
                    validationItems.append({
                        ...
                        "item":
                        self.getErrorItem(
                            "If authentication type is %s, this property is required."
                            % auth_type)
                    })
        ...
        if 'hbase-site' in services['configurations']:
            hbase_zookeeper_quorum = services['configurations']['hbase-site'][
                'properties']['hbase.zookeeper.quorum']
            ...
            elif string_set_equals(
                    application_properties['atlas.graph.storage.hostname'],
                    hbase_zookeeper_quorum):
                validationItems.append({
                    "config-name":
                    "atlas.graph.storage.hostname",
        ...
        validationProblems = self.toConfigurationValidationProblems(
            validationItems, "application-properties")
        return validationProblems

    def validateAtlasConfigurationsFromHDP30(self, properties,
                                             recommendedDefaults,
                                             configurations, services, hosts):
        validationItems = []
        ...
        if not bool(
                re.search(r'^(?=.*[0-9])(?=.*[a-zA-Z]).{8,}$',
                          atlas_admin_password)) or bool(
                              re.search('[\\\`"\']', atlas_admin_password)):
            validationItems.append({
                "config-name":
