...
def install_atlas():
    ...
    Directory([status_params.conf_dir],
    ...
    if not os.path.exists(Script.get_stack_root() + '/' + params.version_dir) or not os.path.exists(
        ...
        Execute('ln -s ' + status_params.conf_dir + ' ' + params.install_dir +
                '/conf')
        Execute("echo 'export PATH=%s/bin:$PATH'>/etc/profile.d/atlas.sh" %
                params.install_dir)
        Execute(
            'chown -R %s:%s %s/%s' % (status_params.metadata_user,
                                        params.user_group, params.stack_root,params.version_dir))
        Execute('chown -R %s:%s %s' % (status_params.metadata_user,
...
def metadata(type='server'):
    ...
    if type == "server":
        ...
        if params.metadata_solrconfig_content:
            File(
                format("{conf_dir}/solr/solrconfig.xml"),
                mode=0644,
                owner=params.metadata_user,
                group=params.user_group,
    ...
    if params.security_enabled:
        TemplateConfig(
            format(params.atlas_jaas_file), owner=params.metadata_user)
    ...
    if type == 'server' and params.search_backend_solr and params.has_infra_solr:
        ...
        if params.security_enabled:
            secure_znode(
                format('{infra_solr_znode}/configs/atlas_configs'), jaasFile)
            secure_znode(
                format('{infra_solr_znode}/collections/vertex_index'),
                jaasFile)
            secure_znode(
                format('{infra_solr_znode}/collections/edge_index'), jaasFile)
            secure_znode(
                format('{infra_solr_znode}/collections/fulltext_index'),
                jaasFile)

    File(
        params.atlas_hbase_setup,
        group=params.user_group,
    ...
    if is_atlas_upgrade_support and params.security_enabled:
        ...
        if not params.host_with_kafka:
            File(
                format("{kafka_conf_dir}/kafka-env.sh"),
                owner=params.kafka_user,
                content=InlineTemplate(params.kafka_env_sh_template))

            File(
                format("{kafka_conf_dir}/kafka_jaas.conf"),
                group=params.user_group,
                owner=params.kafka_user,
