  ...
  """
    ...
    for curr_hostname in atlas_hosts:
        ...
        if "atlas.rest.address" in additional_props:
            additional_props[
                "atlas.rest.address"] += "," + metadata_protocol + "://" + prop_value
        else:
            additional_props[
                "atlas.rest.address"] = metadata_protocol + "://" + prop_value
...
hadoop_conf_dir = os.path.join(
    ...
    "conf") if 'HADOOP_HOME' in os.environ else '/etc/hadoop'
...
# command line args
ssl_enabled = default("/configurations/application-properties/atlas.enableTLS",
                      False)
http_port = default(
    "/configurations/application-properties/atlas.server.http.port", "21000")
https_port = default(
    "/configurations/application-properties/atlas.server.https.port", "21443")
if ssl_enabled:
...
metadata_server_host = atlas_hosts[0] if len(
    atlas_hosts) > 0 else "UNKNOWN_HOST"
...
metadata_host_bind_address_value = default(
    '/configurations/application-properties/atlas.server.bind.address',
    '0.0.0.0')
if metadata_host_bind_address_value == '0.0.0.0':
    ...
    application_properties[
        "atlas.server.bind.address"] = metadata_host_bind_address_value
...
if 'atlas.sso.knox.publicKey' in application_properties:
    ...
    application_properties[
        'atlas.sso.knox.publicKey'] = knox_key_without_new_line
...
is_atlas_ha_enabled = default(
    "/configurations/application-properties/atlas.server.ha.enabled", None)
additional_ha_props = configs_for_ha(atlas_hosts, metadata_port,
                                     is_atlas_ha_enabled, metadata_protocol)
...
atlas_log_max_backup_size = default(
    "/configurations/atlas-log4j/atlas_log_max_backup_size", 256)
atlas_log_number_of_backup_files = default(
    "/configurations/atlas-log4j/atlas_log_number_of_backup_files", 20)
...
# hbase
hbase_conf_dir = default('configurations/atlas-env/atlas.hbase.conf.dir',
                         '/etc/hbase')

atlas_search_backend = default(
    "/configurations/application-properties/atlas.graph.index.search.backend",
    "solr")
...
if 'atlas_solr_replication_factor' in config['configurations']['atlas-env']:
    infra_solr_replication_factor = int(
        default("/configurations/atlas-env/atlas_solr_replication_factor", 1))
...
infra_solr_role_atlas = default(
    'configurations/infra-solr-security-json/infra_solr_role_atlas',
    'atlas_user')
...
atlas_server_max_new_size = default(
    "configurations/atlas-env/atlas_server_max_new_size", 614)
...
hbase_user_keytab = default('/configurations/hbase-env/hbase_user_keytab',
                            None)
hbase_principal_name = default(
    '/configurations/hbase-env/hbase_principal_name', None)

# ToDo: Kafka port to Atlas
# Used while upgrading the stack in a kerberized cluster and running kafka-acls.sh
hosts_with_kafka = default('/clusterHostInfo/kafka_hosts', [])
from random import shuffle
shuffle(hosts_with_kafka)
...
kafka_principal_name = default(
    '/configurations/kafka-env/kafka_principal_name', None)
default_replication_factor = default(
    '/configurations/application-properties/atlas.notification.replicas', None)
...
kafka_zk_endpoint = zk_quorum + '/kafka'

kafka_kerberos_enabled = False
if security_enabled and 'kafka_principal_name' in config[
    ...
    kafka_kerberos_params = "-Djava.security.auth.login.config={0}/kafka_jaas.conf".format(
        kafka_conf_dir)
else:
...
enable_ranger_atlas = default(
    "/configurations/ranger-atlas-plugin-properties/ranger-atlas-plugin-enabled",
    "No")
...
if stack_supports_atlas_ranger_plugin and enable_ranger_atlas:
    ...
    if not has_ranger_admin and enable_ranger_atlas:
        external_admin_username = default(
            '/configurations/ranger-atlas-plugin-properties/external_admin_username',
            'admin')
        external_admin_password = default(
            '/configurations/ranger-atlas-plugin-properties/external_admin_password',
            'admin')
        external_ranger_admin_username = default(
            '/configurations/ranger-atlas-plugin-properties/external_ranger_admin_username',
            'ranger_admin')
        external_ranger_admin_password = default(
            '/configurations/ranger-atlas-plugin-properties/external_ranger_admin_password',
            'example!@#')
        ranger_env = {}
    ...
    ranger_atlas_audit_attrs = config['configurationAttributes'][
        'ranger-atlas-audit']
    ranger_atlas_security = config['configurations']['ranger-atlas-security']
    ranger_atlas_security_attrs = config['configurationAttributes'][
        'ranger-atlas-security']
    ranger_atlas_policymgr_ssl = config['configurations'][
        'ranger-atlas-policymgr-ssl']
    ranger_atlas_policymgr_ssl_attrs = config['configurationAttributes'][
        'ranger-atlas-policymgr-ssl']

    policy_user = config['configurations']['ranger-atlas-plugin-properties'][
        'policy_user']
    ...
    atlas_repository_configuration = {
        ...
        'password':
            unicode(config['configurations']['ranger-atlas-plugin-properties']
    ...
    if security_enabled:
        atlas_repository_configuration[
            'policy.download.auth.users'] = metadata_user
        atlas_repository_configuration[
            'tag.download.auth.users'] = metadata_user
...
with open('/proc/mounts', 'r') as f:
    mounts = [
        line.split()[1] + '/atlas' for line in f.readlines()
