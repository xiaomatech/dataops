...
class HiveServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    """
        if configurations and "hive-site" in configurations and \
            ...
            return configurations["hive-site"]["properties"][
                       "hive.server2.authentication"].lower() == "kerberos"
            ...
            return services["configurations"]["hive-site"]["properties"][
                       "hive.server2.authentication"].lower() == "kerberos"
        else:
            return False
...
class HiveRecommender(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def recommendHiveConfigurationsFromHDP30(self, configurations, clusterData,
                                             services, hosts):
        hiveSiteProperties = self.getSiteProperties(services["configurations"],
                                                    "hive-site")
        hiveEnvProperties = self.getSiteProperties(services["configurations"],
                                                   "hive-env")
        ...
        containerSize = clusterData[
            "mapMemory"] if clusterData["mapMemory"] > 2048 else int(
            clusterData["reduceMemory"])
        containerSize = min(
        ...
        putHiveProperty(
            "hive.tez.java.opts", "-server -Xmx" + str(
                int(round((0.8 * containerSize) + 0.5))) + "m " +
        ...
        if 'DRUID' in servicesList:
            ...
            if 'druid-common' in services['configurations']:
                ...
                druid_metadata_type = services['configurations'][
                    'druid-common']['properties'][
                    'druid.metadata.storage.type']
                if 'druid.metadata.storage.connector.user' in services[
                    'configurations']['druid-common']['properties']:
                    druid_metadata_user = services['configurations'][
                        'druid-common']['properties'][
                        'druid.metadata.storage.connector.user']
                else:
                    druid_metadata_user = ""

            putHiveInteractiveSiteProperty('hive.druid.broker.address.default',
            ...
            putHiveInteractiveSiteProperty(
                ...
                druid_overlord_host_port)
            putHiveInteractiveSiteProperty('hive.druid.metadata.uri',
                                           druid_metadata_uri)
            putHiveInteractiveSiteProperty('hive.druid.metadata.username',
                                           druid_metadata_user)
            putHiveInteractiveSiteProperty('hive.druid.metadata.db.type',
                                           druid_metadata_type)
        ...
        if hiveEnvProperties and hiveSiteProperties and self.checkSiteProperties(
        ...
        if hiveSiteProperties and hiveEnvProperties and self.checkSiteProperties(
                hiveSiteProperties, "ambari.hive.db.schema.name",
                "javax.jdo.option.ConnectionURL") and self.checkSiteProperties(
            hiveEnvProperties, "hive_database"):
            hiveServerHost = self.getHostWithComponent("HIVE", "HIVE_SERVER",
                                                       services, hosts)
            hiveDBConnectionURL = hiveSiteProperties[
                "javax.jdo.option.ConnectionURL"]
            protocol = self.getProtocol(hiveEnvProperties["hive_database"])
            oldSchemaName = self.getOldValue(services, "hive-site",
            ...
            if hiveServerHost is not None:
                if (hiveDBConnectionURL
                    and "//localhost" in hiveDBConnectionURL
                ) or oldSchemaName or oldDBType or (
                        protocol and hiveDBConnectionURL
                        and not hiveDBConnectionURL.startswith(protocol)):
                    dbConnection = self.getDBConnectionString(
                        hiveEnvProperties["hive_database"]).format(
                        hiveServerHost["Hosts"]["host_name"],
                        hiveSiteProperties["ambari.hive.db.schema.name"])
                    putHiveProperty("javax.jdo.option.ConnectionURL",
                                    dbConnection)

        # Transactions
        hs2Hosts = self.getHostsWithComponent("HIVE", "HIVE_SERVER", services,
                                              hosts)
        cpu_count = 0
        ...
        if "yarn-site" in configurations:
            if "yarn.scheduler.minimum-allocation-mb" in configurations[
                ...
                tez_container_size = max(
                    ...
                    int(configurations["yarn-site"]["properties"]
                        ["yarn.scheduler.minimum-allocation-mb"]))
            if "yarn.scheduler.maximum-allocation-mb" in configurations[
                "yarn-site"]["properties"]:
                putHiveSitePropertyAttribute(
                    "hive.tez.container.size", "maximum",
                    int(configurations["yarn-site"]["properties"]
        ...
        tez_container_size_bytes = int(
            int(tez_container_size) * 0.8 * 1024 *
        ...
        if "capacity-scheduler" in services["configurations"]:
            if "capacity-scheduler" in services["configurations"][
                ...
                properties = str(
                    services["configurations"]["capacity-scheduler"]
                    ["properties"]["capacity-scheduler"]).split("\n")
        ...
        putHiveInteractiveSitePropertyAttribute(
            "hive.server2.active.passive.ha.registry.namespace", "visible",
            str(is_hsi_ha).lower())
        ...
        if ("configurations" not in services) or ("hive-env" not in services["configurations"]) or \
            ...
            putHiveEnvProperty(
                "hive_security_authorization", services["configurations"]
                ["hive-env"]["properties"]["hive_security_authorization"])
        ...
        if "ranger-env" in services["configurations"] and "hive-env" in services["configurations"] and \
            ...
            rangerEnvHivePluginProperty = services["configurations"][
                "ranger-env"]["properties"]["ranger-hive-plugin-enabled"]
            rangerEnvHiveAuthProperty = services["configurations"]["hive-env"][
                "properties"]["hive_security_authorization"]
        ...
        if str(configurations["hive-env"]["properties"]
            ...
            if "KERBEROS" not in servicesList:  # Kerberos security depends on this property
                putHiveSiteProperty("hive.security.authorization.enabled",
                                    "false")
        else:
        ...
        if str(configurations["hive-env"]["properties"]
               ["hive_security_authorization"]).lower() == "sqlstdauth":
        ...
        if str(configurations["hive-env"]["properties"]
            ...
            putHiveServerPropertyAttribute("hive.conf.restricted.list",
                                           "delete", "true")

        # Hive authentication
        hive_server2_auth = None
        if "hive-site" in services[
            "configurations"] and "hive.server2.authentication" in services[
            "configurations"]["hive-site"]["properties"]:
            hive_server2_auth = str(
                services["configurations"]["hive-site"]["properties"]
                ["hive.server2.authentication"]).lower()
        elif "hive.server2.authentication" in configurations["hive-site"][
            "properties"]:
            hive_server2_auth = str(configurations["hive-site"]["properties"]
                                    ["hive.server2.authentication"]).lower()
        ...
        if hive_server2_auth == "kerberos":
            if "hive-site" in services[
                "configurations"] and "hive.server2.authentication.kerberos.keytab" not in services[
                "configurations"]["hive-site"]["properties"]:
                putHiveSiteProperty(
                    "hive.server2.authentication.kerberos.keytab", "")
            if "hive-site" in services[
                "configurations"] and "hive.server2.authentication.kerberos.principal" not in services[
                "configurations"]["hive-site"]["properties"]:
                putHiveSiteProperty(
            ...
            if ("hive.server2.authentication.kerberos.keytab" in configurations["hive-site"]["properties"]) or \
                    ...
                    ("hive-site" in services["configurations"] and "hive.server2.authentication.kerberos.keytab" in
                     services["configurations"]["hive-site"]["properties"]):
                putHiveSitePropertyAttribute(
                    "hive.server2.authentication.kerberos.keytab", "delete",
                    "true")
            if ("hive.server2.authentication.kerberos.principal" in configurations["hive-site"]["properties"]) or \
                    ...
                    ("hive-site" in services["configurations"] and "hive.server2.authentication.kerberos.principal" in
                     services["configurations"]["hive-site"]["properties"]):
                putHiveSitePropertyAttribute(
                    "hive.server2.authentication.kerberos.principal", "delete",
                    "true")
        ...
        if hive_server2_auth == "pam":
            ...
            if ("hive.server2.authentication.pam.services" in configurations["hive-site"]["properties"]) or \
                    ...
                    ("hive-site" in services["configurations"] and "hive.server2.authentication.pam.services" in
                     services["configurations"]["hive-site"]["properties"]):
                putHiveSitePropertyAttribute(
                    "hive.server2.authentication.pam.services", "delete",
                    "true")
        ...
        if hive_server2_auth == "custom":
            ...
            if ("hive.server2.authentication" in configurations["hive-site"]["properties"]) or \
                    ...
                    ("hive-site" in services["configurations"] and "hive.server2.custom.authentication.class" in
                     services["configurations"]["hive-site"]["properties"]):
                putHiveSitePropertyAttribute(
                    "hive.server2.custom.authentication.class", "delete",
                    "true")
        ...
        hive_server_hosts = self.getHostsWithComponent("HIVE", "HIVE_SERVER",
                                                       services, hosts)
        hive_client_hosts = self.getHostsWithComponent("HIVE", "HIVE_CLIENT",
                                                       services, hosts)
        ...
        if hive_server_hosts is not None and len(hive_server_hosts):
            ...
            putHiveEnvProperty(
                "hive.heapsize",
                max(512, int(hs_host_ram * hs_heapsize_multiplier)))
            putHiveEnvPropertyAttribute("hive.metastore.heapsize", "maximum",
                                        max(1024, hs_host_ram))
            putHiveEnvPropertyAttribute("hive.heapsize", "maximum",
                                        max(1024, hs_host_ram))
        ...
        if sqla_db_used:
            ...
            putHiveSitePropertyAttribute(
                "datanucleus.rdbms.datastoreAdapterClassName", "delete",
                "true")
        ...
        if "hive-atlas-application.properties" in services[
            "configurations"] and "enable.external.atlas.for.hive" in services[
            "configurations"]["hive-atlas-application.properties"][
            "properties"]:
            enable_external_atlas_for_hive = services["configurations"][
                                                 "hive-atlas-application.properties"]["properties"][
                                                 "enable.external.atlas.for.hive"].lower() == "true"
        ...
        if "hive-env" in configurations and "hive.atlas.hook" in configurations[
            "hive-env"]["properties"]:
            enable_atlas_hook = configurations["hive-env"]["properties"][
                                    "hive.atlas.hook"] == "true"
        elif "hive-env" in services[
            "configurations"] and "hive.atlas.hook" in services[
            "configurations"]["hive-env"]["properties"]:
            enable_atlas_hook = services["configurations"]["hive-env"][
                                    "properties"]["hive.atlas.hook"] == "true"
        ...
        if is_atlas_present_in_cluster and atlas_server_host_info:
            ...
            if "application-properties" in services["configurations"]:
                if "atlas.enableTLS" in services["configurations"][
                    ...
                    tls_enabled = services["configurations"][
                        "application-properties"]["properties"][
                        "atlas.enableTLS"]
                if "atlas.server.http.port" in services["configurations"][
                    "application-properties"]["properties"]:
                ...
                if tls_enabled.lower() == "true":
                    scheme = "https"
                    if "atlas.server.https.port" in services["configurations"][
                        "application-properties"]["properties"]:
                        metadata_port = services["configurations"][
                            "application-properties"]["properties"][
            ...
            putHiveSiteProperty(
                "atlas.rest.address", "{0}://{1}:{2}".format(
                    scheme, atlas_rest_host, metadata_port))
        else:
            putHiveSitePropertyAttribute("atlas.cluster.name", "delete",
                                         "true")
            putHiveSitePropertyAttribute("atlas.rest.address", "delete",
                                         "true")
        ...
        if len(hsi_hosts) > 0:
            ...
            if hsi_properties and "hive.llap.daemon.queue.name" in hsi_properties:
                ...
                hive_tez_default_queue = hsi_properties[
                ...
                if hive_tez_default_queue:
                    ...
                    self.logger.debug(
                        "Updated 'hive.server2.tez.default.queues' config : '{0}'"
                            .format(hive_tez_default_queue))
        else:
            self.logger.info(
        ...
        hive_user = "hive"
        if "hive-env" in services["configurations"] and "hive_user" in services[
            "configurations"]["hive-env"]["properties"]:
        ...
        if "hive-env" in configurations and "hive_security_authorization" in configurations[
            "hive-env"]["properties"]:
            ranger_hive_plugin_enabled = (
                    configurations["hive-env"]["properties"]
                    ["hive_security_authorization"].lower() == "ranger")
        elif "hive-env" in services[
            "configurations"] and "hive_security_authorization" in services[
            "configurations"]["hive-env"]["properties"]:
            ranger_hive_plugin_enabled = (
                    services["configurations"]["hive-env"]["properties"]
                    ["hive_security_authorization"].lower() == "ranger")
        ...
        if ranger_hive_plugin_enabled and "ranger-hive-plugin-properties" in services[
            ...
            putRangerHivePluginProperty("REPOSITORY_CONFIG_USERNAME",
                                        hive_user)
        else:
        ...
        if "hive-atlas-application.properties" in services["configurations"]:
            ...
            if "hive-env" in configurations and "hive.atlas.hook" in configurations[
                "hive-env"]["properties"]:
                enable_atlas_hook = configurations["hive-env"]["properties"][
                                        "hive.atlas.hook"].lower() == "true"
            elif "hive-env" in services[
                "configurations"] and "hive.atlas.hook" in services[
                "configurations"]["hive-env"]["properties"]:
                enable_atlas_hook = services["configurations"]["hive-env"][
                                        "properties"]["hive.atlas.hook"].lower() == "true"
    ...
    def druid_host(self,
                   ...
                   hosts,
                   default_host=None):
        hosts = self.getHostsWithComponent('DRUID', component_name, services,
                                           hosts)
        if hosts and config_type in services['configurations']:
            host = hosts[0]['Hosts']['host_name']
            port = services['configurations'][config_type]['properties'][
                'druid.port']
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [
            ...
            ("hiveserver2-site",
             self.validateHiveServer2ConfigurationsFromHDP30),
    ...
    def validateHiveConfigurationsFromHDP30(self, properties,
                                            recommendedDefaults,
        ...
        if yarn_site:
            ...
            hiveTezContainerSize = self.to_number(
                properties["hive.tez.container.size"])
        ...
        if authentication_property in properties and properties[authentication_property].lower() == "ldap" \
            ...
            validationItems.append({
                ...
                "item":
                    self.getWarnItem(
                        "According to LDAP value for " + authentication_property +
                        ", you should add " + ldap_domain_property +
                        " property, if you are using AD, if not, then " +
                        ldap_baseDN_property + "!")
            })

        hive_enforce_bucketing = "hive.enforce.bucketing"
        if hive_enforce_bucketing in properties and properties[
            hive_enforce_bucketing].lower() == "false":
            validationItems.append({
                "config-name":
                    hive_enforce_bucketing,
                "item":
                    self.getWarnItem(
                        "Set " + hive_enforce_bucketing +
        ...
        if sqla_db_used:
            ...
            elif prop_name in hive_site and hive_site[
                prop_name] != "org.datanucleus.store.rdbms.adapter.SQLAnywhereAdapter":
                validationItems.append({"config-name": prop_name,
                                        "item": self.getWarnItem(
                                            "If Hive using SQL Anywhere db." \
    ...
    def validateHiveConfigurationsEnvFromHDP30(
        ...
        if "hive_security_authorization" in hive_env and \
            ...
            if "RANGER" in servicesList:
                ranger_env = self.getServicesSiteProperties(
                    services, "ranger-env")
        ...
        if "hive.server2.authentication" in hive_site and "LDAP" == hive_site[
            "hive.server2.authentication"]:
            if "alert_ldap_username" not in hive_env or hive_env[
                "alert_ldap_username"] == "":
                validationItems.append({
                    "config-name":
                        "alert_ldap_username",
            ...
            if "alert_ldap_password" not in hive_env or hive_env[
                "alert_ldap_password"] == "":
                validationItems.append({
                    "config-name":
                        "alert_ldap_password",
    ...
    def validateHiveServer2ConfigurationsFromHDP30(
        ...
        ranger_plugin_properties = self.getSiteProperties(
            configurations, "ranger-hive-plugin-properties")
        hive_env_properties = self.getSiteProperties(configurations,
                                                     "hive-env")
        ...
        if ("RANGER" in servicesList):
            ...
            if ranger_plugin_enabled:
                ...
                if prop_name in hive_server2 and hive_server2[
                    prop_name] != prop_val:
                    validationItems.append({"config-name": prop_name,
                                            "item": self.getWarnItem(
    ...
    def validateHiveInteractiveEnvConfigurationsFromHDP30(
        ...
        if len(hsi_hosts) > 0:
            ...
            if "enable_hive_interactive" not in hive_site_env_properties or (
                    ...
                    and hive_site_env_properties["enable_hive_interactive"].
                            lower() != "true"):
                validationItems.append({
                    "config-name":
    ...
    """
        ...
        if hsi_site:
            if "hive.llap.daemon.queue.name" in hsi_site and hsi_site[
                "hive.llap.daemon.queue.name"]:
            ...
            if "hive.server2.enable.doAs" in hsi_site and hsi_site[
        ...
        if llap_queue_name and llap_queue_cap_perc and llap_queue_name == self.AMBARI_MANAGED_LLAP_QUEUE_NAME:
            curr_selected_queue_for_llap_cap = float(
                llap_queue_cap_perc) / 100 * total_cluster_cap
    ...
    """
        ...
        ) and yarn_site_properties and "yarn.nodemanager.resource.memory-mb" in yarn_site_properties:
