...
is_ams_distributed = config['configurations']['ams-site'][
...
random_metric_collector_host = select_metric_collector_hosts_from_hostnames(
    ams_collector_hosts)
...
for host in ams_collector_hosts.split(","):
    metric_truststore_alias = default(
        "/configurations/ams-ssl-client/{host}.ssl.client.truststore.alias",
        None)
...
dashboards_dirs.append(
    os.path.join(agent_cache_dir, service_package_folder, 'files',
                 'grafana-dashboards', stack_name))
# Default
dashboards_dirs.append(
    os.path.join(agent_cache_dir, service_package_folder, 'files',
...
def get_ambari_version():
    ...
    if os.path.exists(AMBARI_AGENT_CONF):
        try:
            ...
            Logger.info(
                'Unable to determine ambari version from version file.')
...
ams_collector_log_dir = config['configurations']['ams-env'][
    'metrics_collector_log_dir']
ams_collector_conf_dir = "/etc/ambari-metrics-collector/conf"
ams_monitor_log_dir = config['configurations']['ams-env'][
...
ams_grafana_protocol = default("/configurations/ams-grafana-ini/protocol",
                               'http')
ams_grafana_cert_file = default("/configurations/ams-grafana-ini/cert_file",
...
security_enabled = False if not is_hbase_distributed else config[
    'configurations']['cluster-env']['security_enabled']
run_kinit = default(
    '/configurations/ams-site/timeline.metrics.run_kinit_before_start', True)
...
metrics_collector_heapsize = default(
    '/configurations/ams-env/metrics_collector_heapsize', "12288")
...
skip_disk_metrics_patterns = default(
    "/configurations/ams-env/timeline.metrics.skip.disk.metrics.patterns",
    None)
skip_network_interfaces_patterns = default(
    "/configurations/ams-env/timeline.metrics.skip.network.interfaces.patterns",
    None)
...
master_heapsize = config['configurations']['ams-hbase-env'][
...
regionserver_heapsize = check_append_heap_property(
    str(regionserver_heapsize), "m")
...
if regionserver_xmn_max:
    regionserver_xmn_max = int(
        trim_heap_property(str(regionserver_xmn_max), "m"))
...
hbase_master_xmn_size = check_append_heap_property(
    str(hbase_master_xmn_size), "m")
regionserver_xmn_size = check_append_heap_property(
    str(regionserver_xmn_size), "m")

# Choose heap size for embedded mode as sum of master + regionserver
if not is_hbase_distributed:
    hbase_heapsize = get_combined_memory_mb(master_heapsize,
...
if security_enabled:
    ...
    hbase_user_keytab = config['configurations']['ams-hbase-env'][
        'hbase_user_keytab']

    ams_collector_jaas_config_file = format(
        "{hbase_conf_dir}/ams_collector_jaas.conf")
    ams_collector_keytab_path = config['configurations'][
        'ams-hbase-security-site']['hbase.myclient.keytab']
    ams_collector_jaas_princ = config['configurations'][
        'ams-hbase-security-site']['hbase.myclient.principal'].replace(
            '_HOST', _hostname_lowercase)

    ams_zookeeper_jaas_config_file = format(
        "{hbase_conf_dir}/ams_zookeeper_jaas.conf")
    ams_zookeeper_keytab = config['configurations']['ams-hbase-security-site'][
        'ams.zookeeper.keytab']
    ams_zookeeper_principal_name = config['configurations'][
        'ams-hbase-security-site']['ams.zookeeper.principal'].replace(
            '_HOST', _hostname_lowercase)
    ...
    master_jaas_princ = config['configurations'][
        'ams-hbase-security-site']['hbase.master.kerberos.principal'].replace(
            '_HOST', _hostname_lowercase)

    regionserver_jaas_config_file = format(
        "{hbase_conf_dir}/hbase_regionserver_jaas.conf")
    regionserver_keytab_path = config['configurations'][
        'ams-hbase-security-site']['hbase.regionserver.keytab.file']
    regionserver_jaas_princ = config['configurations'][
    ...
    if (('ams-hbase-security-site' in config['configurations'])
            and ('ams.monitor.keytab' in config['configurations']
                 ['ams-hbase-security-site'])):
        ams_monitor_keytab = config['configurations'][
            'ams-hbase-security-site']['ams.monitor.keytab']

    ams_monitor_principal = None
    if (('ams-hbase-security-site' in config['configurations'])
            and ('ams.monitor.principal' in config['configurations']
                 ['ams-hbase-security-site'])):
        ams_monitor_principal = config['configurations'][
            'ams-hbase-security-site']['ams.monitor.principal']

    if ams_monitor_keytab and ams_monitor_principal:
        monitor_kinit_cmd = '%s -kt %s %s' % (
            kinit_path_local, ams_monitor_keytab,
...
# log4j.properties
if (('ams-hbase-log4j' in config['configurations'])
        and ('content' in config['configurations']['ams-hbase-log4j'])):
    hbase_log4j_props = config['configurations']['ams-hbase-log4j']['content']
else:
    hbase_log4j_props = None

if (('ams-log4j' in config['configurations'])
        and ('content' in config['configurations']['ams-log4j'])):
...
ams_grafana_env_sh_template = config['configurations']['ams-grafana-env'][
    'content']
ams_grafana_ini_template = config['configurations']['ams-grafana-ini'][
    'content']

hbase_staging_dir = default(
    "/configurations/ams-hbase-site/hbase.bulkload.staging.dir",
    "/ams/hbase/staging")
skip_create_hbase_root_dir = default(
    "/configurations/ams-site/timeline.metrics.skip.create.hbase.root.dir",
    False)
hbase_wal_dir = default("/configurations/ams-hbase-site/hbase.wal.dir", None)
if hbase_wal_dir and re.search(
        "^file://|/",
        hbase_wal_dir):  # If wal dir is on local file system, create it.
...
min_hadoop_sink_version = default(
    "/configurations/ams-env/min_ambari_metrics_hadoop_sink_version",
    "2.7.0.0")
...
with open('/proc/mounts', 'r') as f:
    mounts = [
        line.split()[1] + '/ambari' for line in f.readlines()
