...
class HBASERecommender(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def recommendHbaseConfigurationsFromHDP206(self, configurations,
                                               clusterData, services, hosts):
        ...
        putHbaseProperty('hbase_regionserver_heapsize',
                         max(40960,int(clusterData['hbaseRam']) * 1024))
        putHbaseProperty('hbase_master_heapsize', max(20480,hbaseMasterRam * 1024))

        # recommendations for HBase site config
        putHbaseSiteProperty = self.putProperty(configurations, "hbase-site",
                                                services)
        hbase_zookeeper_hosts = self.getComponentHostNames(services, "HBASE", "HBASE_ZOOKEEPER")
        if hbase_zookeeper_hosts is not None and len(hbase_zookeeper_hosts) > 0:
            hbase_zookeeper_hosts.sort()
            putHbaseSiteProperty("hbase.zookeeper.quorum", ','.join(hbase_zookeeper_hosts))

        if 'hbase-site' in services['configurations'] and 'hbase.superuser' in services['configurations']['hbase-site'][
            'properties'] \
                and 'hbase-env' in services['configurations'] and 'hbase_user' in \
            ...
            putHbaseSiteProperty(
                "hbase.superuser", services['configurations']['hbase-env']
                ['properties']['hbase_user'])
    ...
    """
        if configurations and "hbase-site" in configurations and \
            ...
            return configurations["hbase-site"]["properties"][
                       "hbase.security.authentication"].lower() == "kerberos"
            ...
            return services["configurations"]["hbase-site"]["properties"][
                       "hbase.security.authentication"].lower() == "kerberos"
        else:
            return False

    def recommendHBASEConfigurationsFromHDP22(self, configurations,
                                              clusterData, services, hosts):
        ...
        if rs_hosts is not None and len(rs_hosts) > 0:
            ...
            putHbaseEnvPropertyAttributes('hbase_regionserver_heapsize',
                                          'maximum',
                                          max(40960, int(min_ram * 0.8 / 1024)))
        ...
        if "ranger-env" in services["configurations"] and "ranger-hbase-plugin-properties" in services[
            ...
            rangerEnvHbasePluginProperty = services["configurations"][
                "ranger-env"]["properties"]["ranger-hbase-plugin-enabled"]
            putHbaseRangerPluginProperty("ranger-hbase-plugin-enabled",
                                         rangerEnvHbasePluginProperty)
            if "cluster-env" in services[
                "configurations"] and "smokeuser" in services[
                "configurations"]["cluster-env"]["properties"]:
                smoke_user = services["configurations"]["cluster-env"][
                    "properties"]["smokeuser"]
        ...
        if (int(clusterData["hbaseRam"]) > threshold):
            ...
            if ('hbase_max_direct_memory_size' in configurations["hbase-env"]["properties"]) or \
                    ('hbase-env' in services['configurations'] and 'hbase_max_direct_memory_size' in
                     services['configurations']["hbase-env"]["properties"]):
                putHbaseEnvPropertyAttributes('hbase_max_direct_memory_size',
                                              'delete', 'true')

    def recommendHBASEConfigurationsFromHDP23(self, configurations,
                                              clusterData, services, hosts):
        putHbaseSiteProperty = self.putProperty(configurations, "hbase-site",
                                                services)
        putHbaseSitePropertyAttributes = self.putPropertyAttribute(
    ...
    def recommendHBASEConfigurationsFromHDP26(self, configurations,
                                              clusterData, services, hosts):
        if 'hbase-env' in services[
            'configurations'] and 'hbase_user' in services[
            'configurations']['hbase-env']['properties']:
        ...
        if ranger_hbase_plugin_enabled and 'ranger-hbase-plugin-properties' in services[
            ...
            putRangerHbasePluginProperty = self.putProperty(
                configurations, "ranger-hbase-plugin-properties", services)
            putRangerHbasePluginProperty("REPOSITORY_CONFIG_USERNAME",
                                         hbase_user)
        else:
            self.logger.info("Not setting Hbase Repo user for Ranger.")

    def recommendHBASEConfigurationsFromHDP30(self, configurations,
                                              clusterData, services, hosts):
        ...
        if hbase_atlas_hook_property in configurations['hbase-site'][
            'properties']:
            hbase_master_coprocessor_value = configurations['hbase-site'][
                'properties']['hbase.coprocessor.master.classes']
        else:
            hbase_master_coprocessor_value = 'org.apache.hadoop.hbase.rsgroup.RSGroupAdminEndpoint'
        ...
        if 'hbase-atlas-application-properties' in services[
            'configurations'] and 'enable.external.atlas.for.hbase' in services[
            'configurations']['hbase-atlas-application-properties'][
            'properties']:
            enable_external_hook_for_hbase = services['configurations'][
                                                 'hbase-atlas-application-properties']['properties'][
                                                 'enable.external.atlas.for.hbase'].lower() == 'true'
        else:
        ...
        if 'hbase-env' in configurations and 'hbase.atlas.hook' in configurations[
            'hbase-env']['properties']:
            enable_hbase_atlas_hook = configurations['hbase-env'][
                                          'properties']['hbase.atlas.hook'] == 'true'
        elif 'hbase-env' in services[
            'configurations'] and 'hbase.atlas.hook' in services[
            'configurations']['hbase-env']['properties']:
            enable_hbase_atlas_hook = services['configurations']['hbase-env'][
                                          'properties']['hbase.atlas.hook'] == 'true'
        else:
        ...
        if enable_hbase_atlas_hook:
            ...
            if not is_hbase_atlas_hook_in_config:
                hbase_master_coprocessor_list.append(
        ...
        hbase_master_coprocessor_value = '' if len(
            hbase_master_coprocessor_list) == 0 else ",".join(
            hbase_master_coprocessor_list)
        putHbaseSiteProperty(hbase_atlas_hook_property,
                             hbase_master_coprocessor_value)

    def recommendHBASEConfigurationsFromHDP301(self, configurations,
                                               clusterData, services, hosts):
        # Setters
        putHbaseSiteProperty = self.putProperty(configurations, "hbase-site",
                                                services)
        ...
        parallelGCThreads = int(math.floor(cores / 2)) if cores > 8 else min(
            20, cores)
        ...
        self.setHandlerCounts(configurations, clusterData, services, hosts,
                              cores)

    def setHandlerCounts(self, configurations, clusterData, services, hosts,
                         cores):
        putHbaseSiteProperty = self.putProperty(configurations, "hbase-site",
                                                services)
        ...
        # Is Phoenix enabled?
        phoenix_enabled = 'hbase-env' in services['configurations'] and 'phoenix_sql_enabled' in \
                          services['configurations']['hbase-env']['properties'] and \
                          'true' == services['configurations']['hbase-env']['properties']['phoenix_sql_enabled'].lower()
        ...
        if phoenix_enabled:
            self.logger.info(
                "Setting Phoenix index handlers to %d" % (index_handlers))
            putHbaseSiteProperty('phoenix.rpc.index.handler.count',
                                 index_handlers)

    def recommendHBASEConfigurationsForKerberos(self, configurations,
                                                clusterData, services, hosts):
        putHbaseSiteProperty = self.putProperty(configurations, "hbase-site",
                                                services)
        putHbaseSitePropertyAttributes = self.putPropertyAttribute(
        ...
        is_kerberos_enabled = self.isHBaseKerberosEnabled(
            configurations, services)
        ...
        if is_kerberos_enabled:
            ...
            phoenix_query_server_hosts = self.getPhoenixQueryServerHosts(
                services, hosts)
            self.logger.debug("Calculated Phoenix Query Server hosts: %s" %
        ...
        hbaseCoProcessorConfigs, hbaseCoProcessorConfigAttributes = self.calculateCoprocessorConfigurations(
            services, configurations, ranger_hbase_plugin_enabled,
            is_kerberos_enabled)

        for key in hbaseCoProcessorConfigs:
            putHbaseSiteProperty(key, ','.join(
                set(hbaseCoProcessorConfigs[key])))
    ...
    def calculateCoprocessorConfigurations(self, services, configurations,
        ...
        for key in hbaseCoProcessorConfigs:
            ...
            if key in hbase_site_configurations_properties:
                ...
                hbase_coprocessor_classes = hbase_site_services_properties[
                    key].strip()

            if hbase_coprocessor_classes:
                # Split string into an array with non-empty elements
                hbaseCoProcessorConfigs[key] = filter(
                    None, hbase_coprocessor_classes.split(','))
        ...
        if hbase_security_authorization:
            if 'true' == hbase_security_authorization.lower():
                hbaseCoProcessorConfigs[
                    'hbase.coprocessor.master.classes'].append(
                    'org.apache.hadoop.hbase.security.access.AccessController'
                )
                hbaseCoProcessorConfigs[
                    'hbase.coprocessor.regionserver.classes'].append(
                    'org.apache.hadoop.hbase.security.access.AccessController'
                )
                ...
                for item in range(len(authRegionClasses)):
                    hbaseCoProcessorConfigs[
                        'hbase.coprocessor.region.classes'].append(
                        authRegionClasses[item])
            else:
                if 'org.apache.hadoop.hbase.security.access.AccessController' in hbaseCoProcessorConfigs[
                    'hbase.coprocessor.region.classes']:
                    hbaseCoProcessorConfigs[
        ...
        # Authentication
        if is_kerberos_enabled:
            if 'org.apache.hadoop.hbase.security.access.SecureBulkLoadEndpoint' not in hbaseCoProcessorConfigs[
                'hbase.coprocessor.region.classes']:
                hbaseCoProcessorConfigs[
                    'hbase.coprocessor.region.classes'].append(
                    'org.apache.hadoop.hbase.security.access.SecureBulkLoadEndpoint'
                )
            if 'org.apache.hadoop.hbase.security.token.TokenProvider' not in hbaseCoProcessorConfigs[
                'hbase.coprocessor.region.classes']:
                hbaseCoProcessorConfigs[
                    'hbase.coprocessor.region.classes'].append(
                    'org.apache.hadoop.hbase.security.token.TokenProvider')
        else:
            if 'org.apache.hadoop.hbase.security.token.TokenProvider' in hbaseCoProcessorConfigs[
                'hbase.coprocessor.region.classes']:
                hbaseCoProcessorConfigs[
                    'hbase.coprocessor.region.classes'].remove(
                    'org.apache.hadoop.hbase.security.token.TokenProvider')
        ...
        for key in hbaseCoProcessorConfigs:
            ...
            [
                ...
                if not i in uniqueCoprocessorRegionClassList and (
                    i.strip() not in [
    ...
    """
        if len(hosts['items']) > 0:
            ...
            return [
                ...
                for host in phoenix_query_server_hosts
            ]

    def isRangerPluginEnabled(self, configurations, services):
        if 'ranger-hbase-plugin-properties' in configurations and 'ranger-hbase-plugin-enabled' in configurations[
            'ranger-hbase-plugin-properties']['properties']:
            ranger_hbase_plugin_enabled = (
                    configurations['ranger-hbase-plugin-properties']['properties']
                    ['ranger-hbase-plugin-enabled'].lower() == 'Yes'.lower())
        elif 'ranger-hbase-plugin-properties' in services[
            'configurations'] and 'ranger-hbase-plugin-enabled' in services[
            'configurations']['ranger-hbase-plugin-properties'][
            'properties']:
            ranger_hbase_plugin_enabled = (
                    services['configurations']['ranger-hbase-plugin-properties']
                    ['properties']['ranger-hbase-plugin-enabled'].lower() == 'Yes'.
                    lower())
        else:
...
class HBASEValidator(service_advisor.ServiceAdvisor):
  ...
  """
            ...
            },
            {
                ...
                "item":
                    self.validatorEqualsPropertyItem(
                        properties, "hbase_user", hbase_site,
                        "hbase.superuser")
            }]
    ...
    def validateHBASEConfigurationsFromHDP22(self, properties,
                                             recommendedDefaults,
        ...
        if prop_name1 in hbase_site and not self.is_number(
                hbase_site[prop_name1]):
        ...
        elif prop_name2 in hbase_site and not self.is_number(
                hbase_site[prop_name2]):
            validationItems.append({
                "config-name":
                    prop_name2,
                "item":
        ...
        prop_val = "offheap"
        if prop_name in hbase_site and not (not hbase_site[prop_name] or
                                            hbase_site[prop_name] == prop_val):
        ...
        if prop_name1 in hbase_site and prop_name2 in hbase_site and hbase_site[
            prop_name1] and not hbase_site[prop_name2]:
            validationItems.append({
                "config-name":
                    prop_name2,
                "item":
                    self.getWarnItem(
                        "If bucketcache ioengine is enabled, {0} should be set".
                            format(prop_name2))
            })
        if prop_name1 in hbase_site and prop_name3 in hbase_site and hbase_site[
            prop_name1] and not hbase_site[prop_name3]:
            validationItems.append({
                "config-name":
                    prop_name3,
                "item":
        ...
        if "hbase.security.authentication" in properties:
            hbase_security_kerberos = properties[
                                          "hbase.security.authentication"].lower() == "kerberos"
    ...
    def validateHBASEEnvConfigurationsFromHDP22(
        ...
        hbase_site_properties = self.getSiteProperties(configurations,
                                                       "hbase-site")
        prop_name1 = "hbase.bucketcache.ioengine"

        if prop_name1 in hbase_site_properties and prop_name in hbase_env and hbase_site_properties[
            prop_name1] and hbase_site_properties[
            prop_name1] == "offheap" and not hbase_env[prop_name]:
            validationItems.append({
                "config-name":
                    prop_name,
                "item":
    ...
    def validateHBASERangerPluginConfigurationsFromHDP22(
        ...
        ranger_plugin_enabled = ranger_plugin_properties[
            'ranger-hbase-plugin-enabled'] if ranger_plugin_properties else 'No'
    ...
    def validateHBASEConfigurationsFromHDP23(self, properties,
                                             recommendedDefaults,
                                             configurations, services, hosts):
        hbase_site = properties
        validationItems = []
        ...
        if ("RANGER" in servicesList) and (
            ...
            if (prop_val in hbase_site[prop_name]
                ...
                validationItems.append({"config-name": prop_name,
                                        "item": self.getWarnItem(
                                            "If Ranger HBase Plugin is enabled." \
                                            " {0} needs to contain {1} instead of {2}".format(prop_name, prop_val,
                                                                                              exclude_val))})
