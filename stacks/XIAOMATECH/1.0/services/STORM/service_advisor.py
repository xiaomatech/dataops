...
class StormServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def getServiceConfigurationRecommendations(self, configurations,
        ...
        recommender.recommendStormConfigurationsFromHDP206(
            configurations, clusterData, services, hosts)
        recommender.recommendStormConfigurationsFromHDP21(
            configurations, clusterData, services, hosts)
        recommender.recommendStormConfigurationsFromHDP22(
            configurations, clusterData, services, hosts)
        recommender.recommendStormConfigurationsFromHDP23(
            configurations, clusterData, services, hosts)
        recommender.recommendStormConfigurationsFromHDP26(
            configurations, clusterData, services, hosts)
        recommender.recommendStormConfigurationsFromHDP30(
            configurations, clusterData, services, hosts)
    ...
    def isKerberosEnabled(services, configurations):
        ...
        if configurations and "storm-site" in configurations and \
            ...
            return configurations["storm-site"]["properties"][
            ...
            return services["configurations"]["storm-site"]["properties"][
                "storm.thrift.transport"] == "org.apache.storm.security.auth.kerberos.KerberosSaslTransportPlugin"
        else:
            return False
...
class StormRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendStormConfigurationsFromHDP206(self, configurations,
                                               clusterData, services, hosts):
    ...
    def recommendStormConfigurationsFromHDP21(self, configurations,
                                              clusterData, services, hosts):
        storm_mounts = [("storm.local.dir", ["NODEMANAGER", "NIMBUS"],
                         "/hadoop/storm", "single")]

        self.updateMountProperties("storm-site", storm_mounts, configurations,
                                   services, hosts)

    def recommendStormConfigurationsFromHDP22(self, configurations,
                                              clusterData, services, hosts):
        putStormSiteProperty = self.putProperty(configurations, "storm-site",
                                                services)
        ...
        security_enabled = StormServiceAdvisor.isKerberosEnabled(
        ...
        if "ranger-env" in services["configurations"] and "ranger-storm-plugin-properties" in services[
            ...
            putStormRangerPluginProperty = self.putProperty(
                configurations, "ranger-storm-plugin-properties", services)
            rangerEnvStormPluginProperty = services["configurations"][
                "ranger-env"]["properties"]["ranger-storm-plugin-enabled"]
            putStormRangerPluginProperty("ranger-storm-plugin-enabled",
                                         rangerEnvStormPluginProperty)
    ...
    def recommendStormConfigurationsFromHDP23(self, configurations,
                                              clusterData, services, hosts):
        putStormStartupProperty = self.putProperty(configurations,
                                                   "storm-site", services)
        putStormEnvProperty = self.putProperty(configurations, "storm-env",
                                               services)
        servicesList = [
            service["StackServices"]["service_name"]
            for service in services["services"]
        ...
        if "storm-site" in services["configurations"]:
            ...
            if notifier_plugin_property in services["configurations"]["storm-site"]["properties"] and \
                ...
                notifier_plugin_value = services["configurations"][
                    "storm-site"]["properties"][notifier_plugin_property]
            else:
            ...
            if 'storm-atlas-application.properties' in services[
                'configurations'] and 'enable.external.atlas.for.storm' in \
                    services['configurations']['storm-atlas-application.properties']['properties']:
                enable_external_atlas_for_storm = \
                    services['configurations']['storm-atlas-application.properties']['properties'][
                        'enable.external.atlas.for.storm'].lower() == "true"
            ...
            if 'storm-env' in configurations and 'storm.atlas.hook' in configurations[
                    'storm-env']['properties']:
                enable_atlas_hook = configurations['storm-env']['properties'][
                ...
                enable_atlas_hook = services['configurations']['storm-env'][
                    'properties']['storm.atlas.hook'] == "true"
            ...
            if not enable_atlas_hook and atlas_hook_is_set:
                application_classes = [
                    item for item in notifier_plugin_value.split(",")
                    if item != atlas_hook_class and item != " "
                ]
                notifier_plugin_value = ",".join(
                    application_classes) if application_classes else " "

            if notifier_plugin_value.strip() != "":
                putStormStartupProperty(notifier_plugin_property,
                                        notifier_plugin_value)
            else:
                putStormStartupPropertyAttribute = self.putPropertyAttribute(
                    configurations, "storm-site")
                putStormStartupPropertyAttribute(notifier_plugin_property,
                                                 'delete', 'true')

    def recommendStormConfigurationsFromHDP26(self, configurations,
                                              clusterData, services, hosts):
        """
    ...
    def recommendStormConfigurationsFromHDP30(self, configurations,
                                              clusterData, services, hosts):
        ...
        if storm_site and not security_enabled:
            putStormSitePropertyAttribute = self.putPropertyAttribute(
                configurations, "storm-site")
            if 'nimbus.impersonation.acl' in storm_site:
                putStormSitePropertyAttribute('nimbus.impersonation.acl',
                                              'delete', 'true')
            if 'nimbus.impersonation.authorizer' in storm_site:
                putStormSitePropertyAttribute(
                    'nimbus.impersonation.authorizer', 'delete', 'true')
...
class StormValidator(service_advisor.ServiceAdvisor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [("ranger-storm-plugin-properties",
                            self.validateStormConfigurationsFromHDP22)]


    def validateStormConfigurationsFromHDP22(self, properties,
                                             recommendedDefaults,
        ...
        ranger_plugin_enabled = ranger_plugin_properties[
            'ranger-storm-plugin-enabled'] if ranger_plugin_properties else 'No'
        servicesList = [
            service["StackServices"]["service_name"]
            for service in services["services"]
        ]
        ...
        ) == 'Yes'.lower()) and not security_enabled:
            validationItems.append({
                "config-name":
                "ranger-storm-plugin-enabled",
                "item":
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "ranger-storm-plugin-properties")
