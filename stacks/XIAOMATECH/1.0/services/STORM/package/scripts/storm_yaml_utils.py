        ...
        if security_enabled:
            if name.find('Nimbus_JVM') > -1:
                return name.replace(
                    ...
                    '/storm_jaas.conf -Djavax.security.auth.useSubjectCredsOnly=false'
                )
            else:
                return name.replace(
                    '_JAAS_PLACEHOLDER', '-Djava.security.auth.login.config=' +
...
def yaml_config_template(configurations):
    return InlineTemplate(
        ...
        extra_imports=[
            escape_yaml_property, replace_jaas_placeholder,
...
{% endfor %}''',
        ...
        extra_imports=[
            escape_yaml_property, resource_management,
