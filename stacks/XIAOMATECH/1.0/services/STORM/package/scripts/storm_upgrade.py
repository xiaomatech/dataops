...
class StormUpgrade(Script):
    ...
    def delete_storm_zookeeper_data(self, env):
        ...
        if storm_zookeeper_server_list is None:
            Logger.info(
                "Unable to extract ZooKeeper hosts from '{0}', assuming localhost"
            ).format(params.storm_zookeeper_servers)
        ...
        for storm_zookeeper_server in storm_zookeeper_server_list:
            ...
            zk_command_location = os.path.join(params.stack_root, "zookeeper",
                                               "bin", "zkCli.sh")
            if params.version is not None:
                zk_command_location = os.path.join(
                    params.stack_root, "zookeeper", "bin", "zkCli.sh")
