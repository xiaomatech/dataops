    ...
    CELERY_ANNOTATIONS = {'tasks.add': {'rate_limit': '10/s'}}        
        ...
        PropertiesFile(
            ...
            properties=quote_string_values(
                superset_config, params.non_quoted_configs, CeleryConfig),
            owner=params.superset_user,
            group=params.user_group)
        ...
        if len(params.druid_coordinator_hosts) > 0:
            ...
            generate_logfeeder_input_config(
                'superset',
                Template(
                    "input.config-superset.json.j2", extra_imports=[default]))
...
def quote_string_value(value, non_quoted_configs):
    if value.isdigit() or value.lower() == "true" or value.lower(
    ) == "false" or value in non_quoted_configs:
        return value
    else:
