...
def get_tokens():
  ...
  :return tuple
  """
    return (RANGER_ADMIN_URL, ADMIN_USERNAME, ADMIN_PASSWORD,
            RANGER_ADMIN_USERNAME, RANGER_ADMIN_PASSWORD, SECURITY_ENABLED)
...
def check_ranger_login(ranger_auth_link, username, password):
  ...
  """
    try:
        ...
        logger.exception(
            "Error during Ranger service authentication. {0}".format(e.reason))
        return None
    except Exception, e:
        return 401
...
def get_ranger_user(ranger_get_user, username, password, user):
  ...
  """
    try:
        ...
        logger.exception(
            "Error getting user from Ranger service. Http status code - {0}. {1}"
            .format(e.code, e.read()))
        return False
    except urllib2.URLError, e:
        logger.exception("Error getting user from Ranger service. {0}".format(
            e.reason))
        return False
    except Exception, e:
        return False
