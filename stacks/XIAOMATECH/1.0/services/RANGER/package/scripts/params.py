...
ugsync_principal = config['configurations'][
    'ranger-ugsync-site']['ranger.usersync.kerberos.principal'].replace(
...
install_dir_usersync = Script.get_stack_root() + "/ranger-usersync"
download_url_usersync = default(
    "/configurations/ranger-env/download_url_usersync", "")
filename_usersync = download_url_usersync.split('/')[-1]
version_dir_usersync = filename_usersync.replace('.tar.gz', '').replace(
    '.tgz', '')

install_dir_tagsync = Script.get_stack_root() +"/ranger-tagsync"
download_url_tagsync = default(
    "/configurations/ranger-env/download_url_tagsync", "")
filename_tagsync = download_url_tagsync.split('/')[-1]
version_dir_tagsync = filename_tagsync.replace('.tar.gz', '').replace(
    '.tgz', '')
...
stack_supports_multiple_env_sh_files = True

upgrade_direction = default("/commandParams/upgrade_direction", None)

ranger_home = install_dir_admin
ranger_conf = '/etc/ranger-admin'
ranger_admin_services_file = format(
    '{stack_root}/ranger-admin/ews/ranger-admin-services.sh')

usersync_home = format('{stack_root}/ranger-usersync')
ranger_ugsync_conf = '/etc/ranger-usersync'
usersync_services_file = format(
    '{stack_root}/ranger-usersync/ranger-usersync-services.sh')

ranger_tagsync_home = format('{stack_root}/ranger-tagsync')
ranger_tagsync_conf = '/etc/ranger-tagsync/'
tagsync_services_file = format(
    '{stack_root}/ranger-tagsync/ranger-tagsync-services.sh')

security_store_path = '/etc/security/serverKeys'
tagsync_etc_path = '/etc/ranger-tagsync'
ranger_tagsync_credential_file = os.path.join(tagsync_etc_path,
                                              'rangercred.jceks')
atlas_tagsync_credential_file = os.path.join(tagsync_etc_path,
                                             'atlascred.jceks')
...
atlas_tagsync_keystore_password = config['configurations'][
    'atlas-tagsync-ssl']['xasecure.policymgr.clientssl.keystore.password']
atlas_tagsync_truststore_password = config['configurations'][
    'atlas-tagsync-ssl']['xasecure.policymgr.clientssl.truststore.password']
...
usersync_log_dir = default(
    "/configurations/ranger-env/ranger_usersync_log_dir",
    "/var/log/ranger/usersync")
admin_log_dir = default("/configurations/ranger-env/ranger_admin_log_dir",
                        "/var/log/ranger/admin")
ranger_admin_default_file = format(
    '{ranger_conf}/ranger-admin-default-site.xml')
security_app_context_file = format(
    '{ranger_conf}/security-applicationContext.xml')
ranger_ugsync_default_file = format(
    '{ranger_ugsync_conf}/ranger-ugsync-default.xml')
...
if db_flavor.lower() == 'mysql':
    ...
    audit_jdbc_url = format(
        'jdbc:sqlanywhere:database={ranger_auditdb_name};host={db_host}'
    ) if stack_supports_ranger_audit_db else None
...
if stack_supports_config_versioning:
    ...
    previous_jdbc_jar = format(
        "{ranger_home}/ews/lib/{previous_jdbc_jar_name}")
...
ranger_credential_provider_path = config["configurations"][
    "ranger-admin-site"]["ranger.credential.provider.path"]
ranger_jpa_jdbc_credential_alias = config["configurations"][
    "ranger-admin-site"]["ranger.jpa.jdbc.credential.alias"]
ranger_ambari_db_password = unicode(
    config["configurations"]["admin-properties"]["db_password"])
...
if not is_empty(config["configurations"]["admin-properties"]
    ...
    ranger_ambari_audit_db_password = unicode(
...
cred_setup_prefix = (format('{ranger_home}/ranger_credential_helper.py'), '-l',
                     cred_lib_path)
...
ranger_ug_ldap_user_searchfilter = config["configurations"][
    "ranger-ugsync-site"]["ranger.usersync.ldap.user.searchfilter"]
ranger_ug_ldap_group_searchbase = config["configurations"][
    "ranger-ugsync-site"]["ranger.usersync.group.searchbase"]
ranger_ug_ldap_group_searchfilter = config["configurations"][
    "ranger-ugsync-site"]["ranger.usersync.group.searchfilter"]
...
tagsync_log_dir = default(
    "/configurations/ranger-tagsync-site/ranger.tagsync.logdir",
...
tagsync_application_properties = dict(
    ...
    ["tagsync-application-properties"]) if has_ranger_tagsync else None
tagsync_pid_file = format('{ranger_pid_dir}/tagsync.pid')
tagsync_cred_lib = os.path.join(ranger_tagsync_home, "lib", "*")

ranger_usersync_log_maxfilesize = default(
    '/configurations/usersync-log4j/ranger_usersync_log_maxfilesize', 256)
ranger_usersync_log_maxbackupindex = default(
    '/configurations/usersync-log4j/ranger_usersync_log_maxbackupindex', 20)
ranger_tagsync_log_maxfilesize = default(
    '/configurations/tagsync-log4j/ranger_tagsync_log_maxfilesize', 256)
ranger_tagsync_log_number_of_backup_files = default(
    '/configurations/tagsync-log4j/ranger_tagsync_log_number_of_backup_files',
    20)
ranger_xa_log_maxfilesize = default(
    '/configurations/admin-log4j/ranger_xa_log_maxfilesize', 256)
ranger_xa_log_maxbackupindex = default(
    '/configurations/admin-log4j/ranger_xa_log_maxbackupindex', 20)
...
ranger_solr_shards = config['configurations']['ranger-env'][
    'ranger_solr_shards']
...
is_solrCloud_enabled = default(
    '/configurations/ranger-env/is_solrCloud_enabled', False)
...
if stack_supports_infra_client and is_solrCloud_enabled:
    solr_znode = default(
        '/configurations/ranger-admin-site/ranger.audit.solr.zookeepers',
        'NONE')
    ...
    if has_infra_solr and not is_external_solrCloud_enabled:
        solr_znode = config['configurations']['infra-solr-env'][
            'infra_solr_znode']
solr_user = unix_user
if has_infra_solr and not is_external_solrCloud_enabled:
    solr_user = default('/configurations/infra-solr-env/infra_solr_user',
                        unix_user)
...
ranger_audit_max_retention_days = config['configurations'][
    'ranger-solr-configuration']['ranger_audit_max_retention_days']
ranger_audit_logs_merge_factor = config['configurations'][
    'ranger-solr-configuration']['ranger_audit_logs_merge_factor']
ranger_solr_config_content = config['configurations'][
    'ranger-solr-configuration']['content']
...
if security_enabled:
    if has_ranger_tagsync:
        ranger_tagsync_principal = config['configurations'][
            'ranger-tagsync-site']['ranger.tagsync.kerberos.principal']
        if not is_empty(
                ranger_tagsync_principal) and ranger_tagsync_principal != '':
    ...
    if stack_supports_ranger_kerberos:
        ...
        if not is_empty(
            ...
            if stack_supports_infra_client and is_solrCloud_enabled and is_external_solrCloud_enabled and is_external_solrCloud_kerberos:
                solr_jaas_file = format(
                    '{ranger_home}/conf/ranger_solr_jaas.conf')
    ...
    if security_enabled:
        ...
        if has_ranger_tagsync:
            if not is_empty(ranger_tagsync_principal
                ...
                ranger_tagsync_bare_principal = get_bare_principal(
                    ranger_tagsync_principal)
            rule_dict.append({
                'principal': ranger_tagsync_bare_principal,
                'user': 'rangertagsync'
            })
        ...
        for item in range(len(rule_dict)):
            rule_line = 'RULE:[2:$1@$0]({0}@{1})s/.*/{2}/\n'.format(
                rule_dict[item]['principal'], realm, rule_dict[item]['user'])
        ...
        core_site_property[
            'hadoop.security.auth_to_local'] = core_site_auth_to_local_property
...
if is_hbase_ha_enabled:
    if not is_empty(config['configurations']['ranger-hbase-plugin-properties']
                    ['ranger-hbase-plugin-enabled']):
        ranger_hbase_plugin_enabled = config['configurations'][
            'ranger-hbase-plugin-properties'][
                'ranger-hbase-plugin-enabled'].lower() == 'yes'
if is_namenode_ha_enabled:
    if not is_empty(config['configurations']['ranger-hdfs-plugin-properties']
                    ['ranger-hdfs-plugin-enabled']):
        ranger_hdfs_plugin_enabled = config['configurations'][
            'ranger-hdfs-plugin-properties'][
                'ranger-hdfs-plugin-enabled'].lower() == 'yes'

ranger_admin_password_properties = [
    'ranger.jpa.jdbc.password', 'ranger.jpa.audit.jdbc.password',
...
ranger_truststore_alias = default(
    '/configurations/ranger-admin-site/ranger.truststore.alias',
    'trustStoreAlias')
...
is_ranger_usersync_host = 'role' in config and config[
    'role'] == "RANGER_USERSYNC"
