...
# Tez jars
tez_local_api_jars = params.stack_root + '/tez/tez*.jar'
tez_local_lib_jars = params.stack_root + '/tez/lib/*.jar'
...
def install_tez_jars():
    ...
    destination_hdfs_dirs = get_tez_hdfs_dir_paths(tez_lib_uris)
    ...
    if destination_hdfs_dirs:
        ...
        tez_jars = {}
        if app_dir_path:
            tez_jars[tez_local_api_jars] = app_dir_path
        if lib_dir_path:
            tez_jars[tez_local_lib_jars] = lib_dir_path
    ...
    if tez_lib_uris and tez_lib_uris.strip().find(hdfs_path_prefix, 0) != -1:
        ...
        for path in dir_paths:
            if not "tez.tar.gz" in path:
                ...
                lib_dir_path = lib_dir_path if lib_dir_path.endswith(
                    os.sep) else lib_dir_path + os.sep
