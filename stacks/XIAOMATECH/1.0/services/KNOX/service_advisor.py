...
class KnoxServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def getServiceConfigurationRecommendations(self, configurations,
        ...
        recommender.recommendKnoxConfigurationsFromHDP22(
...
class KnoxRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendKnoxConfigurationsFromHDP22(self, configurations, clusterData,
                                             services, hosts):
        if "ranger-env" in services["configurations"] and "ranger-knox-plugin-properties" in services[
            ...
            putKnoxRangerPluginProperty = self.putProperty(
                configurations, "ranger-knox-plugin-properties", services)
            rangerEnvKnoxPluginProperty = services["configurations"][
                "ranger-env"]["properties"]["ranger-knox-plugin-enabled"]
            putKnoxRangerPluginProperty("ranger-knox-plugin-enabled",
                                        rangerEnvKnoxPluginProperty)

        if 'topology' in services["configurations"] and 'content' in services[
                "configurations"]["topology"]["properties"]:
            putKnoxTopologyContent = self.putProperty(configurations,
                                                      "topology", services)
            rangerPluginEnabled = ''
            if 'ranger-knox-plugin-properties' in configurations and 'ranger-knox-plugin-enabled' in \
                ...
                rangerPluginEnabled = configurations[
                    'ranger-knox-plugin-properties']['properties'][
            ...
            topologyContent = services["configurations"]["topology"][
                "properties"]["content"]
            ...
            if root is not None:
                ...
                if gateway is not None:
                    for provider in gateway.findall('provider'):
                        ...
                        if role is not None and role.text and role.text.lower(
                        ) == "authorization":
                        ...
                        if authNameChanged:
                            ...
                            putKnoxTopologyContent('content',
                                                   ET.tostring(root))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [
            ("ranger-knox-plugin-properties",
             self.validateKnoxRangerPluginConfigurationsFromHDP22),
        ]

    def validateKnoxRangerPluginConfigurationsFromHDP22(
        ...
        ranger_plugin_properties = self.getSiteProperties(
            configurations, "ranger-knox-plugin-properties")
        ranger_plugin_enabled = ranger_plugin_properties[
            'ranger-knox-plugin-enabled'] if ranger_plugin_properties else 'No'
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "ranger-knox-plugin-properties")
