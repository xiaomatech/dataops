...
def append_xml_content(original_content, conf_dict,
                       property_prefix="content-"):
    ...
    for k, v in conf_dict.items():
        if k.startswith(property_prefix):
            try:
                ...
                additional_config_root = ElementTree.fromstring(
                    evaluated_additional_value)
                original_config_root.append(additional_config_root)
            except ElementTree.ParseError:
                Logger.error(
                    "Failed to evaluate property \"{0}\" to a valid xml, its content will not be appended to target "
                    .format(k) +
