...
class ServiceCheck(Script):
    def service_check(self, env):
        ...
        if params.registry_ssl_enabled:
            registry_api = format(
                "https://{params.hostname}:{params.registry_ssl_port}/api/v1/schemaregistry/schemaproviders"
            )
        else:
            registry_api = format(
                "http://{params.hostname}:{params.registry_port}/api/v1/schemaregistry/schemaproviders"
            )
        ...
        if params.security_enabled:
            kinit_cmd = format(
                "{kinit_path_local} -kt {params.smoke_user_keytab} {params.smokeuser_principal};"
            )
            ...
            try:
                ...
                if (params.security_enabled or params.registry_ssl_enabled):
                    ...
                    try:
                        ...
                        Logger.info(
                            format(
                                "Successfully made a API request to registry. {stdout}"
                            ))
                        break
                    except Exception as e:
                        Logger.error(
                            format(
                                "Response from REGISTRY API was not a valid JSON. Response: {stdout}"
                            ))
                else:
                    ...
                    Logger.info(
                        format(
                            "registry response http status {response_code}"))
                    if response.getcode() != 200:
                        Logger.error(
                            format(
                                "Failed to fetch response for {registry_api}"))
                        show_logs(params.registry_log_dir,
                                  params.registry_user)
                    else:
                        success = True
                        Logger.info(
                            format(
                                "Successfully made a API request to registry. {api_response}"
                            ))
                        break
        ...
        if success != True:
            raise Fail(
                format(
                    "Failed to make API request to Registry server at {registry_api} after {max_retries}"
                ))
