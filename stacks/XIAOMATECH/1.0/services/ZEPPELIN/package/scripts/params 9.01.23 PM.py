...
install_dir = stack_root + '/zeppelin'
...
zeppelin_port = str(
    config['configurations']['zeppelin-site']['zeppelin.server.port'])
if is_ui_ssl_enabled:
    zeppelin_port = str(
        config['configurations']['zeppelin-site']['zeppelin.server.ssl.port'])
zeppelin_interpreter = None
if 'zeppelin.interpreter.group.order' in config['configurations'][
    'zeppelin-site']:
...
zeppelin_group = config['configurations']['cluster-env']['user_group']
...
zeppelin_dir = os.path.join(*[install_dir])
...
# zeppelin-env.sh
zeppelin_env_content = config['configurations']['zeppelin-env'][
    'zeppelin_env_content']

# shiro.ini
shiro_ini_content = config['configurations']['zeppelin-shiro-ini'][
    'shiro_ini_content']

# log4j.properties
log4j_properties_content = config['configurations'][
    'zeppelin-log4j-properties']['log4j_properties_content']
...
if 'hive_server_hosts' in master_configs and len(
        master_configs['hive_server_hosts']) != 0:
    ...
    hive_server2_support_dynamic_service_discovery = config['configurations'][
        'hive-site']['hive.server2.support.dynamic.service.discovery']

hive_server_interactive_hosts = None
if 'hive_server_interactive_hosts' in master_configs and len(
...
if 'hive-site' in config['configurations']:
    if 'hive.server2.authentication.kerberos.principal' in config[
...
if 'spark2-hive-site-override' in config[
    'configurations'] and 'hive.server2.transport.mode' in config[
    'configurations']['spark2-hive-site-override']:
    spark2_transport_mode = config['configurations'][
        'spark2-hive-site-override']['hive.server2.transport.mode']

if 'spark_thriftserver_hosts' in master_configs and len(
        master_configs['spark_thriftserver_hosts']) != 0:
    spark_thrift_server_hosts = str(
        master_configs['spark_thriftserver_hosts'][0])
    if config['configurations']['spark-hive-site-override']:
        spark_hive_thrift_port = config['configurations'][
...
if 'spark2_thriftserver_hosts' in master_configs and len(
        master_configs['spark2_thriftserver_hosts']) != 0:
    spark2_thrift_server_hosts = str(
        master_configs['spark2_thriftserver_hosts'][0])
    if config['configurations']['spark2-hive-site-override']:
        spark2_hive_thrift_port = config['configurations'][
            'spark2-hive-site-override']['hive.server2.thrift.port']
        if 'hive.server2.authentication.kerberos.principal' in config[
            'configurations']['spark2-hive-site-override']:
            spark2_hive_principal = config['configurations'][
...
if 'hbase_master_hosts' in master_configs and 'hbase-site' in config[
    'configurations']:
...
# detect spark queue
if 'spark-defaults' in config[
    'configurations'] and 'spark.yarn.queue' in config['configurations'][
    'spark-defaults']:
    spark_queue = config['configurations']['spark-defaults'][
        'spark.yarn.queue']
elif 'spark2-defaults' in config[
    'configurations'] and 'spark.yarn.queue' in config['configurations'][
    'spark2-defaults']:
...
if 'zeppelin.interpreter.config.upgrade' in config['configurations'][
    'zeppelin-site']:
    zeppelin_interpreter_config_upgrade = config['configurations'][
        'zeppelin-site']['zeppelin.interpreter.config.upgrade']
else:
