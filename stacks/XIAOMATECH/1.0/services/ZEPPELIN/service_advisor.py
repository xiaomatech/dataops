...
class ZeppelinServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    """
        if configurations and "zeppelin-env" in configurations and \
            ...
            return configurations["zeppelin-env"]["properties"][
                "zeppelin.kerberos.enabled"].lower() == "true"
            ...
            return services["configurations"]["zeppelin-env"]["properties"][
                "zeppelin.kerberos.enabled"].lower() == "true"
        else:
            return False
            ...
            if zeppelin_site and 'zeppelin.server.kerberos.principal' in zeppelin_site:
                ...
                if zeppelin_user:
                    self.__conditionallyUpdateSuperUsers(
                        'livy2-conf', 'livy.superusers', zeppelin_user,
                        configurations, services)

    def __conditionallyUpdateSuperUsers(self, config_name, property_name,
                                        user_to_add, configurations, services):
        config = self.getServicesSiteProperties(services, config_name)

        if config:
            superusers = config[
                property_name] if property_name in config else None
            ...
            if superusers:
                ...
                _superusers = filter(
                    None,
                    _superusers)  # Removes empty string elements from array
            else:
                _superusers = []
            ...
            if user_to_add not in _superusers:
                ...
                putProperty = self.putProperty(configurations, config_name,
                                               services)
