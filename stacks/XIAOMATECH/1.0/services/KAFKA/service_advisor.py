...
class KafkaServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def isKerberosEnabled(services, configurations):
        ...
        if configurations and "kafka-broker" in configurations and \
            ...
            return configurations["kafka-broker"]["properties"][
            ...
            return services["configurations"]["kafka-broker"]["properties"][
                       "security.inter.broker.protocol"] == "SASL_PLAINTEXT"
        else:
            return False
...
class KafkaRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendKafkaConfigurationsFromHDP22(self, configurations,
                                              clusterData, services, hosts):
    ...
    def recommendKAFKAConfigurationsFromHDP23(self, configurations,
                                              clusterData, services, hosts):
        ...
        security_enabled = KafkaServiceAdvisor.isKerberosEnabled(
        ...
        putKafkaLog4jProperty = self.putProperty(configurations, "kafka-log4j",
                                                 services)
        putKafkaBrokerAttributes = self.putPropertyAttribute(
            configurations, "kafka-broker")
        ...
        if "ranger-env" in services["configurations"] \
            ...
            ranger_kafka_plugin_enabled = services["configurations"][
                "ranger-env"]["properties"]["ranger-kafka-plugin-enabled"]
        ...
        if "RANGER" in servicesList:
            ...
            if 'ranger-kafka-plugin-properties' in configurations and \
                ...
                ranger_plugin_enabled = configurations[
                                            'ranger-kafka-plugin-properties']['properties'][
            ...
            # determine if the Ranger/Kafka plug-in enabled enabled or not
                    ...
                    'ranger-kafka-plugin-enabled' in services['configurations']['ranger-kafka-plugin-properties'][
                'properties']:
                ranger_plugin_enabled = services['configurations'][
                                            'ranger-kafka-plugin-properties']['properties'][
                                            'ranger-kafka-plugin-enabled'].lower() == 'yes'
        ...
        if ranger_plugin_enabled:
            kafkaLog4jRangerLines = [
                ...
                {
                    "name":
                        "log4j.appender.rangerAppender.layout.ConversionPattern",
                    "value":
                        "%d{ISO8601} %p [%t] %C{6} (%F:%L) - %m%n"
                },
                {
            ...
            if 'kafka-log4j' in services[
                'configurations'] and 'content' in services[
                'configurations']['kafka-log4j']['properties']:
                kafkaLog4jContent = services['configurations']['kafka-log4j'][
                    'properties']['content']
                for item in range(len(kafkaLog4jRangerLines)):
                    if kafkaLog4jRangerLines[item][
                        "name"] not in kafkaLog4jContent:
            ...
            if zookeeper_host_port:
                putRangerKafkaPluginProperty = self.putProperty(
                    configurations, 'ranger-kafka-plugin-properties', services)
                putRangerKafkaPluginProperty('zookeeper.connect',
                                             zookeeper_host_port)

    def update_listeners_to_sasl(self, services, putKafkaBrokerProperty):
        try:
            listeners = services['configurations']['kafka-broker'][
            ...
            self.logger.info(
                'Cannot replace PLAINTEXT to SASL_PLAINTEXT in listeners. KeyError: %s'
                % e)

    def recommendKAFKAConfigurationsFromHDP26(self, configurations,
                                              clusterData, services, hosts):
        if 'kafka-env' in services[
            'configurations'] and 'kafka_user' in services[
            'configurations']['kafka-env']['properties']:
            kafka_user = services['configurations']['kafka-env']['properties'][
                'kafka_user']
        else:
            kafka_user = "kafka"

        if 'ranger-kafka-plugin-properties' in configurations and 'ranger-kafka-plugin-enabled' in \
                configurations['ranger-kafka-plugin-properties']['properties']:
            ranger_kafka_plugin_enabled = (
                    configurations['ranger-kafka-plugin-properties']['properties']
                    ['ranger-kafka-plugin-enabled'].lower() == 'Yes'.lower())
        elif 'ranger-kafka-plugin-properties' in services['configurations'] and 'ranger-kafka-plugin-enabled' in \
                services['configurations']['ranger-kafka-plugin-properties']['properties']:
            ranger_kafka_plugin_enabled = (
                    services['configurations']['ranger-kafka-plugin-properties']
                    ['properties']['ranger-kafka-plugin-enabled'].lower() == 'Yes'.
                    lower())
        else:
            ranger_kafka_plugin_enabled = False

        if ranger_kafka_plugin_enabled and 'ranger-kafka-plugin-properties' in services[
            'configurations'] and 'REPOSITORY_CONFIG_USERNAME' in \
                services['configurations']['ranger-kafka-plugin-properties']['properties']:
            self.logger.info("Setting Kafka Repo user for Ranger.")
            putRangerKafkaPluginProperty = self.putProperty(
                configurations, "ranger-kafka-plugin-properties", services)
            putRangerKafkaPluginProperty("REPOSITORY_CONFIG_USERNAME",
                                         kafka_user)
        else:
            self.logger.info("Not setting Kafka Repo user for Ranger.")

    def recommendKAFKAConfigurationsFromHDP30(self, configurations,
                                              clusterData, services, hosts):
        ...
        for each_service in range(0, number_services):
            if services['services'][each_service]['components'][0][
                'StackServiceComponents']['service_name'] == 'KAFKA':
                ...
                putKafkaBrokerProperty('offsets.topic.replication.factor',
                                       str(min(3, num_kakfa_brokers)))
            if services['services'][each_service]['components'][0][
                'StackServiceComponents'][
                'service_name'] == 'STREAMSMSGMGR':
...
class KafkaValidator(service_advisor.ServiceAdvisor):
    ...
    def validateKafkaRangerPluginConfigurationsFromHDP22(
        ...
        ranger_plugin_enabled = ranger_plugin_properties[
            'ranger-kafka-plugin-enabled'] if ranger_plugin_properties else 'No'
        servicesList = [
            service["StackServices"]["service_name"]
            for service in services["services"]
        ]
    ...
    def validateKAFKAConfigurationsFromHDP23(self, properties,
                                             recommendedDefaults,
                                             configurations, services, hosts):
        ...
        kafka_broker_properties = self.getSiteProperties(
            configurations, "kafka-broker")
        ...
        for each_service in range(0, number_services):
            if services['services'][each_service]['components'][0][
                ...
                num_kakfa_brokers = len(
                    services['services'][each_service]['components'][0]
                    ['StackServiceComponents']['hostnames'])
                if int(kafka_broker_properties[
                    ...
                    validationItems.append({"config-name": 'offsets.topic.replication.factor',
                                            "item": self.getErrorItem(
                                                ...
                                                "It must be less or same as number of kafka brokers." \
                                                    .format(kafka_broker_properties['offsets.topic.replication.factor'],
                                                            num_kakfa_brokers))})
    ...
    def validateKAFKAConfigurationsFromHDP30(self, properties,
                                             recommendedDefaults,
                                             configurations, services, hosts):
        kafka_broker = properties
        validationItems = []
        ...
        if ("STREAMSMSGMGR" in servicesList):
            ...
            prop_val = 'true'
            try:
                kafka_broker_producer_metrics_enable = kafka_broker_properties[
                    prop_name]
                if kafka_broker_producer_metrics_enable.lower() == "false":
                    validationItems.append({"config-name": prop_name,
                                            "item": self.getWarnItem(
                                                ...
                                                "count by Kafka brokers.".format(prop_name,
                                                                                 kafka_broker_producer_metrics_enable))})
            except KeyError as e:
                self.logger.info("Unable to find producer.metrics.enable.")
                validationItems.append({"config-name": prop_name,
                                        "item": self.getWarnItem(
                                            "After Installing Streams Messaging Manager, {0} with be set to {1}." \
                                            "Please restart kafka for changes to take place.".format(prop_name,
                                                                                                     prop_val))})
