...
class ServiceCheck(Script):
    def service_check(self, env):
        ...
        if topic_exists_cmd_code > 0:
            raise Fail("Error encountered when attempting to list topics: {0}".
                       format(topic_exists_cmd_out))
        ...
        if under_rep_cmd_code > 0:
            raise Fail(
                "Error encountered when attempting find under replicated partitions: {0}"
                .format(under_rep_cmd_out))
        elif len(under_rep_cmd_out) > 0 and "Topic" in under_rep_cmd_out:
            Logger.warning("Under replicated partitions found: {0}".format(
                under_rep_cmd_out))
