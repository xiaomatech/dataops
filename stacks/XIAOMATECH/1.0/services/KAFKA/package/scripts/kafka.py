...
def install_kafka():
    ...
    if not os.path.exists(Script.get_stack_root() + '/' + params.version_dir) or not os.path.exists(
        ...
        Execute('ln -s ' + params.conf_dir + ' ' + params.install_dir +
                '/config')
        Execute("echo 'export PATH=%s/bin:$PATH'>/etc/profile.d/kafka.sh" %
                params.install_dir)
        Execute('chown -R %s:%s %s/%s' %
                (params.kafka_user, params.user_group,params.stack_root, params.version_dir))
        Execute('chown -R %s:%s %s' % (params.kafka_user, params.user_group,
                                       params.install_dir))
