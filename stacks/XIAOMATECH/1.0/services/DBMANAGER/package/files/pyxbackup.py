...
cmd_no_log = [
    XB_CMD_LIST, XB_CMD_STAT, XB_CMD_META, XB_CMD_BINLOGS, XB_CMD_PRUNE
]
...
def _xb_version(verstr=None, tof=False):
    ...
    if verstr is None:
        if XB_VERSION is not None:
            if tof:
                return float("%d.%d" % (XB_VERSION_MAJOR, XB_VERSION_MINOR))
            else:
                return True
        ...
        XB_VERSION = "%d.%d.%d" % (XB_VERSION_MAJOR, XB_VERSION_MINOR,
                                   XB_VERSION_REV)

        if XB_VERSION_MAJOR == 0:
            _error("Invalid xtrabackup version or unable to determine valid "
                   "version string or binary version non-GA release")
            _error("Version string was \"%s\"" % err)
            _die("Exiting")

        if XB_VERSION_MAJOR >= 3: xb_ibx_bin = 'xtrabackup'
...
def _check_in_progress():
    ...
    if os.path.isfile(XB_LCK_FILE):
        _debug("%s lock file exists and is_backup is %s" % (XB_LCK_FILE,
                                                            str(is_backup)))
        ...
        if is_backup:
            try:
                ...
                if e.errno == errno.ESRCH:
                    _die("%s lock file exists but process is not running" %
                         XB_LCK_FILE)
...
def _apply_log(bkp, incrdir=None, final=False):
    ...
    if cfp.get(XB_BIN_NAME, 'backup_type') == 'incremental':
        ...
        if XB_VERSION_MAJOR >= 3:
            ibx_opts += " --incremental-dir %s --target-dir %s" % (bkp,
                                                                   incrdir)
        else:
    ...
    try:
        ...
        if r != 0: raise Exception("Non-zero exit of xtrabackup command!")
...
def _prepare_backup(bkp, prep, final=False):
    ...
    if is_cmp:
        ...
        for fmt in [
                'xbs.gz', 'tar.gz', 'xbs.qp', 'xbs.qp.xbcrypt', 'qp',
                'qp.xbcrypt'
        ]:
        ...
        if not _decompress(bkp_file, cmp_to, meta):
            _die("An error occurred while extracting %s to %s" % (bkp_file,
                                                                  cmp_to))
...
def _compress_tgz(bkp, tgz):
    ...
    if os.path.isfile(tgz):
        _warn("Destination archive already exists, ", tgz,
...
def _extract_xgz(xgz, dest):
    ...
    if r != 0:
        ...
        _error(
            "Extract returned exit codes were: %s and %s" % (str(r), str(x)))
        _exit_code(XB_EXIT_EXTRACT_FAIL)
        return False
...
def _extract_xbcrypt(dest, meta=None):
    ...
    if _xb_version(tof=True) < 2.3:
        _say("You are running an older xtrabackup version "
...
def _extract_stream_qpress(xbs, dest, meta=None):
    ...
    if xbs[-14:] == 'xbs.qp.xbcrypt':
        if not xb_opt_encrypt:
            _die("Backup is %s is encrypted, no encryption options provided" %
                 qp)
        else:
            xbc_cmd = 'xbcrypt --decrypt --encrypt-algo=%s ' % xb_opt_encrypt
            xbc_cmd += '--encrypt-key-file=%s --input=%s' % (
                xb_opt_encrypt_key_file, xbs)
...
def _extract_nostream_qpress(qp, dest, meta=None):
    ...
    if is_encrypted:
        ...
        if r != 0:
            ...
            _error("Extract returned exit codes were: %s and %s" % (str(r),
                                                                    str(x)))
            _exit_code(XB_EXIT_EXTRACT_FAIL)
            return False
    else:
...
def _ssh_execute(cmd, out=False, nowait=False):
    ...
    if xb_opt_debug:
        ...
        ssh_cmd = "ssh -o PasswordAuthentication=no -q"
...
def _pre_run_xb():
    ...
    if xb_opt_remote_push_only:
        t = _ssh_execute(
            "%s -q --meta-item=xb_last_backup,xb_last_backup_is,xb_last_full meta"
            % xb_opt_remote_script)
        if t: t = t.split(' ')
        if len(t) < 3:
...
def _purge_binlogs():
    ...
    if has_gaps:
        _warn(
            "Stored binary logs has gaps in index series, usability in PITR is at risk"
        )

    if len(binlog_names) > 1:
        _warn(
            "Stored binary logs has multiple name prefixes, we will not be able to purge!"
        )
        ...
        for l in xb_binlogs_list:
            ...
            if prev is not None:
                ...
                # We keep from this binlog and keep the previous one as well
                    ...
                    _say(
                        "%s matches binary log retention period, stopping" % f)
                    break
            else:
                prev = f
...
def _stream_binlog_from():
    ...
    def _missing_from_server(bname, bindex, start_from_name=None):
        if start_from_name is not None:
            _warn(
                "%s was determined from backups/--first-binlog, but this file "
    ...
    if start_from is not None:
        ...
        if xb_stor_start_binlog is not None and xb_opt_binlog_resume == 1:
            ...
            if low[0:-7] == xb_stor_end_binlog[0:-7] and start_from is None:
                ...
                _warn(
                    "Existing binary log backups have different prefix name than source server"
                )
    ...
    if start_from < int(low[-6:]) or start_from is None:
        if xb_opt_binlog_resume == 1:
            start_from, start_from_name = _missing_from_server(
                low[0:-7], int(low[-6:]), start_from_name)
            _warn("Streaming ALL from %s with --binlog-resume=1" %
                  start_from_name)
        else:
            start_from = None
        ...
        if xb_opt_binlog_resume == 1:
            start_from, start_from_name = _missing_from_server(
                high[0:-7], int(high[-6:]), start_from_name)
            _warn(
                "Streaming from last file %s from the server with --binlog-resume=1"
                % start_from_name)
        else:
            start_from = None
    ...
    if start_from is None:
        if start_from_name is not None:
            _die(
                ...
                "--binlog-resume=1 to automatically identify possible start files."
                % start_from_name)
        else:
            _die(
...
def _find_open_port():
    ...
    for p in range(xb_opt_remote_nc_port_min, xb_opt_remote_nc_port_max + 1):
        ...
        awk_cmd = Popen([awk, ' {print $4}'],
                        stdin=netstat_cmd.stdout,
                        stdout=PIPE)
...
def _open_remote_nc_port(port, pipe_cmd):
    ...
    for i in range(1, 4):
        _ssh_execute("nc -l %d | %s" % (port, pipe_cmd), nowait=True)
        time.sleep(5)
...
def _close_remote_nc_port(port):
    ...
    for i in range(1, 4):
        _ssh_execute(
            "echo 'FORCE_CLOSE' | nc localhost %d" % port, nowait=True)
        time.sleep(3)
...
def run_wipeout():
    ...
    dirs = [
        xb_stor_full, xb_stor_incr, xb_stor_weekly, xb_stor_monthly,
        xb_stor_binlogs,
        os.path.join(xb_opt_stor_dir, 'tmp'), xb_opt_work_dir
    ]
...
def run_xb():
    ...
    try:
        ...
        ibx_log = "%s/%s-innobackupex-backup.log" % (xb_opt_work_dir,
                                                     xb_curdate)
        tee_cmd = "tee %s" % ibx_log
        ...
        if xb_opt_compress and not xb_opt_apply_log:
            ...
            if xb_opt_remote_push_only:
                backup_archive = "%s/%s" % (xb_this_backup_remote,
                                            backup_fname)
            else:
            ...
            if xb_opt_remote_push_only:
                if xb_opt_remote_nc_port_min:
                    pipe_cmd = "nc %s %d" % (xb_opt_remote_host,
                                             xb_opt_remote_nc_port_min)
                else:
                    pipe_cmd = "ssh -o PasswordAuthentication=no -q %s %s@%s '%s'" % (
                        xb_opt_ssh_opts, xb_opt_ssh_user, xb_opt_remote_host,
                        pipe_cmd)
    ...
    if xb_opt_apply_log and xb_backup_is_success:
        ...
        if xb_opt_command == XB_CMD_FULL:
            ...
            xb_prepare_is_success = _apply_log(xb_this_backup,
                                               xb_prepared_backup)
    ...
    if xb_opt_remote_host:
        ...
        if xb_opt_remote_nc_port_min:
            ...
            _push_to_remote_scp(xb_this_backup,
...
def run_xb_full():
    ...
    if xb_opt_remote_host:
        xb_this_backup_remote = os.path.join(xb_opt_remote_stor_dir, 'full',
                                             xb_curdate)
...
def run_xb_incr():
    ...
    if xb_opt_remote_push_only:
        ...
        if not os.path.isdir(ib): os.mkdir(ib)
        pull_from_remote(
            os.path.join(rb, XB_CKP_FILE), os.path.join(ib, XB_CKP_FILE))
    ...
    if xb_opt_remote_host:
        xb_this_backup_remote = os.path.join(xb_opt_remote_stor_dir, 'incr',
                                             xb_last_full, xb_curdate)
...
def run_status():
    ...
    if xb_backup_in_progress is not None:
        ...
        if pid <= 0:
            ...
            try:
                ...
                if last_dt < old_dt:
                    ...
                    txt = "Backup process in progress with PID %d" % pid
        ...
        if last_dt < old_dt:
            ...
            txt = "Last backup %s from %s" % (xb_last_backup_is,
                                              xb_last_backup)
    ...
    if xb_opt_status_format == 'nagios':
        print txt
    elif xb_opt_status_format == 'zabbix':
        print ret
    sys.exit(ret)
...
def run_xb_restore_set(prepare_path=None, finalize=True):
    ...
    if backup_is == XB_CMD_FULL:
        ...
        if not prepare_success:
            _die("There was a problem preparing full backup %s" %
                 the_backup_path)
        ...
        for current_bkp in current_incr_list:
            current_bkp_path = os.path.join(xb_stor_incr, bkp_full,
                                            current_bkp)

            if current_bkp == the_backup and finalize:
                prepare_success = _prepare_backup(current_bkp_path,
                                                  prepare_path, finalize)
            else:
                prepare_success = _prepare_backup(current_bkp_path,
                                                  prepare_path)
...
def run_xb_apply_last():
    ...
    if xb_last_full not in xb_incr_list or xb_incr_list[xb_last_full] is None:
        _say("No incremental backups taken for %s, we're done for now" %
             xb_last_full)
        return True
...
def run_binlog_stream():
    ...
    if xb_opt_binlog_binary is not None:
        if not os.path.isfile(xb_opt_binlog_binary):
            ...
            mysqlbinlog = xb_opt_binlog_binary
    else:
        mysqlbinlog = _which('mysqlbinlog')
    ...
    try:
        ...
        while True:
            ...
            pid_file = os.path.join('/tmp',
                                    "%s-binlog-stream.pid" % XB_BIN_NAME)
            ...
            if r != 0:
                _error(
                    "mysqlbinlog command failed with error code %s" % str(r))
...
"""
    ...
    parser.add_option(
        ...
        type='string',
        help=
        ('By default, config options are read from the %s section. '
    ...
    parser.add_option(
        '-b',
        '--retention-binlogs',
        dest='retention_binlogs',
        type="int",
        help='Binary log period retention, in days')
    parser.add_option(
        '',
        '--extra-ibx-options',
        dest='extra_ibx_options',
        type='string',
    ...
    parser.add_option(
        ...
        dest='ssh_user',
        help=
        'SSH account to user when streaming backups to remote host, default is root'
    )
    parser.add_option(
        '-x',
        '--apply-log',
        ...
        action="store_true",
        help=
        'Verify backups with --apply-log, requires enough disk space on --workdir'
    )
    parser.add_option(
        '-m',
        '--prepare-memory',
        ...
        type="int",
        help=
        'How much memory to use with innobackupex --use-memory in MB, default 128M'
    )
    parser.add_option(
        '-o',
        '--status-format',
    ...
    parser.add_option(
        ...
        type="string",
        help=
        ('With command restore-set, specify which backup to restore, '
    ...
    parser.add_option(
        ...
        type="string",
        help=
        'With command restore, specify where to restore selected backup (cli)')
    parser.add_option(
        '-i',
        '--retention-sets',
        dest='retention_sets',
        help=
        'How many sets of combined full + incr to keep on storage, default 2')
    parser.add_option(
        '-j',
        '--retention-months',
        dest='retention_months',
        type="int",
    ...
    parser.add_option(
        ...
        type="string",
        help=
        ('Query meta information about backups, used when backups '
    ...
    parser.add_option(
        '',
        '--binlog-from-master',
        dest='binlog_from_master',
        action="store_true",
        help=
        ('For binlog-stream, when --slave-info is enabled on the backups '
    ...
    parser.add_option(
        ...
        action="store_true",
        help=
        'Supress all messages errors except intended output i.e. list command (cli)',
        default=False)
    parser.add_option(
        '-X',
        '--i-am-absolutely-sure-wipeout',
        dest='wipeout',
        action="store_true",
    ...
    if os.path.isfile(xb_opt_config):
        ...
        if xb_cfg.has_option(xb_opt_config_section, 'mysql_port'):
            xb_opt_mysql_port = int(
                xb_cfg.get(xb_opt_config_section, 'mysql_port'))
        ...
        if xb_cfg.has_option(xb_opt_config_section, 'stor_dir'):
            xb_opt_stor_dir = xb_cfg.get(xb_opt_config_section,
                                         'stor_dir').rstrip('/')

        if xb_cfg.has_option(xb_opt_config_section, 'work_dir'):
            xb_opt_work_dir = xb_cfg.get(xb_opt_config_section,
                                         'work_dir').rstrip('/')
        ...
        if xb_cfg.has_option(xb_opt_config_section, 'remote_stor_dir'):
            xb_opt_remote_stor_dir = xb_cfg.get(xb_opt_config_section,
                                                'remote_stor_dir').rstrip('/')

        if xb_cfg.has_option(xb_opt_config_section, 'remote_host'):
            xb_opt_remote_host = xb_cfg.get(xb_opt_config_section,
                                            'remote_host')

        if xb_cfg.has_option(xb_opt_config_section, 'remote_script'):
            xb_opt_remote_script = xb_cfg.get(xb_opt_config_section,
                                              'remote_script')

        if xb_cfg.has_option(xb_opt_config_section, 'remote_push_only'):
            xb_opt_remote_push_only = bool(
                int(xb_cfg.get(xb_opt_config_section, 'remote_push_only')))

        if xb_cfg.has_option(xb_opt_config_section, 'remote_nc_port'):
            if not _parse_port_param(
                    xb_cfg.get(xb_opt_config_section, 'remote_nc_port')):
                parser.error("The specified port (range) is not valid")
            else:
                xb_opt_remote_nc_port = xb_cfg.get(xb_opt_config_section,
                                                   'remote_nc_port')

        if xb_cfg.has_option(xb_opt_config_section, 'retention_binlogs'):
            xb_opt_retention_binlogs = int(
                xb_cfg.get(xb_opt_config_section, 'retention_binlogs'))

        if xb_cfg.has_option(xb_opt_config_section, 'binlog_binary'):
            xb_opt_binlog_binary = xb_cfg.get(xb_opt_config_section,
                                              'binlog_binary')

        if xb_cfg.has_option(xb_opt_config_section, 'binlog_from_master'):
            xb_opt_binlog_from_master = xb_cfg.get(xb_opt_config_section,
                                                   'binlog_from_master')

        if xb_cfg.has_option(xb_opt_config_section, 'binlog_resume'):
            xb_opt_binlog_resume = xb_cfg.get(xb_opt_config_section,
                                              'binlog_resume')

        if xb_cfg.has_option(xb_opt_config_section, 'first_binlog'):
            xb_opt_first_binlog = xb_cfg.get(xb_opt_config_section,
                                             'first_binlog')

        if xb_cfg.has_option(xb_opt_config_section, 'compress'):
            xb_opt_compress = bool(
                int(xb_cfg.get(xb_opt_config_section, 'compress')))

        if xb_cfg.has_option(xb_opt_config_section, 'compress_with'):
            xb_opt_compress_with = xb_cfg.get(xb_opt_config_section,
                                              'compress_with')

        if xb_cfg.has_option(xb_opt_config_section, 'notify_by_email'):
            xb_opt_notify_by_email = xb_cfg.get(xb_opt_config_section,
                                                'notify_by_email')

        if xb_cfg.has_option(xb_opt_config_section, 'notify_on_success'):
            xb_opt_notify_on_success = xb_cfg.get(xb_opt_config_section,
                                                  'notify_on_success')

        if xb_cfg.has_option(xb_opt_config_section, 'apply_log'):
            xb_opt_apply_log = bool(
                int(xb_cfg.get(xb_opt_config_section, 'apply_log')))

        if xb_cfg.has_option(xb_opt_config_section, 'prepare_memory'):
            xb_opt_prepare_memory = int(
                xb_cfg.get(xb_opt_config_section, 'prepare_memory'))

        if xb_cfg.has_option(xb_opt_config_section, 'retention_sets'):
            if int(xb_cfg.get(xb_opt_config_section, 'retention_sets')) > 0:
                xb_opt_retention_sets = int(
                    xb_cfg.get(xb_opt_config_section, 'retention_sets'))

        if xb_cfg.has_option(xb_opt_config_section, 'retention_months'):
            if int(xb_cfg.get(xb_opt_config_section, 'retention_months')) > 0:
                xb_opt_retention_months = int(
                    xb_cfg.get(xb_opt_config_section, 'retention_months'))

        if xb_cfg.has_option(xb_opt_config_section, 'retention_weeks'):
            if int(xb_cfg.get(xb_opt_config_section, 'retention_weeks')) > 0:
                xb_opt_retention_weeks = int(
                    xb_cfg.get(xb_opt_config_section, 'retention_weeks'))

        if xb_cfg.has_option(xb_opt_config_section, 'encrypt_key_file'):
            xb_opt_encrypt_key_file = xb_cfg.get(xb_opt_config_section,
                                                 'encrypt_key_file')
        ...
        if xb_cfg.has_option(xb_opt_config_section, 'extra_ibx_options'):
            xb_opt_extra_ibx_options = xb_cfg.get(xb_opt_config_section,
                                                  'extra_ibx_options')

        if xb_cfg.has_option(xb_opt_config_section, 'purge_bitmaps'):
            xb_opt_purge_bitmaps = xb_cfg.get(xb_opt_config_section,
                                              'purge_bitmaps')
    ...
    if options.retention_binlogs:
        xb_opt_retention_binlogs = options.retention_binlogs
    if options.compress: xb_opt_compress = options.compress
    if options.compress_with: xb_opt_compress_with = options.compress_with
    if options.notify_by_email:
        xb_opt_notify_by_email = options.notify_by_email
    if options.notify_on_success:
        xb_opt_notify_on_success = options.notify_on_success
    ...
    if options.binlog_from_master:
        xb_opt_binlog_from_master = options.binlog_from_master

    xb_opt_binlog_resume = options.binlog_resume

    if options.remote_stor_dir:
        xb_opt_remote_stor_dir = options.remote_stor_dir
    ...
    if xb_opt_remote_host is not None and xb_opt_remote_stor_dir is None:
        parser.error(
            "Remote host specified but, remote store directory is empty")
    ...
    if options.encrypt_key_file:
        xb_opt_encrypt_key_file = options.encrypt_key_file
    ...
    cmds = [
        XB_CMD_FULL, XB_CMD_INCR, XB_CMD_LIST, XB_CMD_STAT, XB_CMD_PREP,
        XB_CMD_APPL, XB_CMD_PRUNE, XB_CMD_META, XB_CMD_BINLOGS, XB_CMD_WIPE
    ]
    if len(args) >= 1 and args[0] not in cmds:
        parser.error(
            "Command not recognized, got '%s'. See more with --help" % args[0])
    elif len(args) <= 0:
...
def check_dirs():
    ...
    if not os.path.isdir(xb_opt_stor_dir):
        _die("The store directory \"%s\" is not a valid directory" %
             xb_opt_stor_dir)

    if not os.path.isdir(xb_opt_work_dir):
        _die("The work directory \"%s\" is not a valid directory" %
             xb_opt_work_dir)
...
def list_backups():
    ...
    if len(
            l
    ) <= 0 and xb_opt_command == XB_CMD_INCR and not xb_opt_remote_push_only:
        _exit_code(XB_EXIT_NO_FULL)
        ...
        for d in xb_full_list:
            ...
            if d not in xb_full_list:
                _debug("A group of incremental backup from the folder ",
                       os.path.join(xb_stor_incr, d),
                       " has no parent backup from ", xb_stor_full)
            ...
            for r in i[:]:
                if not os.path.isfile(
                        os.path.join(xb_stor_incr, d, r, XB_TAG_FILE)):
                    _debug("Incremental backup ",
                           os.path.join(xb_stor_incr, d, r),
...
if __name__ == "__main__":
    try:
        ...
        if xb_opt_command not in cmd_no_log:
            ...
            _init_log_file(
                "%s/%s-%s" % (xb_opt_work_dir, xb_curdate, XB_LOG_NAME))
        ...
        if os.path.isfile(xb_log_file):
            if xb_opt_remote_host and xb_opt_command not in [
                    XB_CMD_PREP, XB_CMD_APPL
            ]:
                _push_to_remote_scp(xb_log_file,
                                    "%s/" % xb_this_backup_remote.rstrip('/'))
        ...
        if xb_exit_code > 0 and xb_opt_notify_by_email:
            _notify_by_email(
                "MySQL backup script at %s has errors!" % xb_hostname)
        elif xb_opt_notify_on_success and xb_opt_command in [
                XB_CMD_FULL, XB_CMD_INCR
        ]:
            _notify_by_email(
                "MySQL backup script at %s completed successfully!" %
                xb_hostname, xb_backup_summary, xb_opt_notify_on_success)
...
class PyxOptions(object):
    ...
    def __init__(self):
        ...
        parser.add_option(
            ...
            type='string',
            help=
            'Path to config file to use, useful for multiple back locations')
        parser.add_option(
            '',
            '--config-section',
            dest='config_section',
        ...
        parser.add_option(
            ...
            type='string',
            help=(
                'Path to custom my.cnf, in case you want to pass this value to '
                'innobackupex --defaults-file'))
        parser.add_option(
            '-s',
            '--stor-dir',
        ...
        parser.add_option(
            ...
            dest='compress_with',
            help=
            'Compress backup with binary, default gzip, options (gzip, qpress)'
        )
        parser.add_option(
            '-M',
            '--notify-by-email',
        ...
        parser.add_option(
            ...
            dest='remote_stor_dir',
            help=(
                'When --remote-host is not empty, backups to that host will be '
                'streamed to this directory, similar to --stor-dir'))
        parser.add_option(
            '-T',
            '--remote-host',
        ...
        parser.add_option(
            ...
            dest='remote_nc_port',
            help=
            ('When requesting to open a netcat port, this is the port number '
             'to try with, can be a range separated with comma'))
        parser.add_option(
            '-C',
            '--ssh-opts',
        ...
        parser.add_option(
            ...
            type="string",
            help=(
                'For binlog-stream, specify where the 5.6+ mysqlbinlog utility '
                'is located'))
        parser.add_option(
            '-d',
            '--debug',
        ...
        parser.add_option(
            ...
            action="store_true",
            help=
            'Confirm to **WIPEOUT** all backups with wipeout command! (cli)',
            default=False)
        parser.add_option(
            '',
            '--encrypt',
        ...
        parser.add_option(
            ...
            action="store_true",
            help=
            ('If Changed Page Tracking is enabled, should we automatically '
             'purge bitmaps? Requires that a valid mysql-user and mysql-pass '
             'with SUPER privieleges is specified.'))
        ...
        self.config = "/etc/%s.cnf" % XB_BIN_NAME
        if not os.path.isfile(self.config):
            self.config = "%s/%s.cnf" % (xb_cwd, XB_BIN_NAME)
        self.config_section = XB_BIN_NAME
        ...
        if options.config_section:
            self.config_section = options.config_section
        cfg = self.read_config_file(self.config, self.config_section)
        ...
        if options.retention_binlogs:
            retention_binlogs = options.retention_binlogs
        ...
        if self.remote_host is not None and self.remote_stor_dir is None:
            parser.error(
                "Remote host specified but, remote store directory is empty")
        ...
        if len(args) >= 1 and args[0] not in cmds:
            parser.error(
                "Command not recognized, got '%s'. See more with --help" %
                args[0])
        elif len(args) <= 0:
            parser.error("Command not specified. See more with --help")
        else:
        ...
        if self.remote_push_only and self.apply_log:
            _die("--remote-push-only and --apply-log are mutually exclusive")
        ...
        if self.encrypt and not os.path.isfile(self.encrypt_key_file):
            _die("The specified key file does not exist!")

        if self.encrypt and self.compress and self.compress_with == 'gzip':
            _die("GZIP compression + encryption is not supported ",
                 "at the moment. Please use --compress-with=qpress instead.")

        if self.encrypt and not self.compress:
            _die("Encryption requires compression for now, support for ",
                 "uncompressed encrypted backup will be added in the future")

        if self.command in [
                XB_CMD_FULL, XB_CMD_INCR, XB_CMD_PREP, XB_CMD_APPL
        ]:
            _check_binary('innobackupex')
            _check_binary('xtrabackup')

        if self.remote_nc_port_min:
            _check_binary('nc')
            _check_binary('netstat')

        # store xtrabackup version numbers
        if self.command not in [XB_CMD_WIPE, XB_CMD_LIST]:
            _xb_version()

        # we test email delivery beforehand to make sure it works
        # this will happen only once as long as the sentinel file exists
        # i.e. STOR_DIR/pyxbackup_mail_ok
        mail_status_file = "%s/%s_mail_ok" % (self.stor_dir, XB_BIN_NAME)
        if (self.notify_by_email or self.notify_on_success) and \
                not os.path.isfile(mail_status_file):
            mail_message = "This is a test message from %s@%s, please ignore." % (
                xb_user, xb_hostname)
            mail_subject = "pyxbackup Test Mail"
            mail_to = self.notify_by_email \
                if self.notify_by_email else self.notify_on_success
        ...
        if self.debug:
            ...
            _debug("\tcommand: %s" % self.command)

    def read_config_file(self, cfg_file, config_section):
        cfg = ConfigParser()
        cfg.read(cfg_file)
        ...
        if cfg.has_option(config_section, 'remote_stor_dir'):
            remote_stor_dir = cfg.get(config_section,
                                      'remote_stor_dir').rstrip('/')
        ...
        if cfg.has_option(config_section, 'remote_push_only'):
            remote_push_only = bool(
                int(cfg.get(config_section, 'remote_push_only')))
        ...
        if cfg.has_option(config_section, 'retention_binlogs'):
            retention_binlogs = int(
                cfg.get(config_section, 'retention_binlogs'))
