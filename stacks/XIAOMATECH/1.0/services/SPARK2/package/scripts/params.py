...
spark_warehouse_dir = config['configurations']['spark2-defaults'][
    "spark.sql.warehouse.dir"]
whs_dir_protocol = urlparse(spark_warehouse_dir).scheme
default_metastore_catalog = config['configurations'][
    'spark2-hive-site-override']["metastore.catalog.default"]
...
spark_hadoop_lib_native = format(
    "{hadoop_home}/lib/native") + ':/usr/share/java/hadoop/native/'
...
graphite_host = ''
graphite_hosts = default("/clusterHostInfo/graphouse_hosts", [])
graphite_count = len(graphite_hosts)
if graphite_count > 0:
    graphite_host = graphite_hosts[random.randint(0, graphite_count - 1)]

# spark-defaults params
ui_ssl_enabled = default("configurations/spark2-defaults/spark.ssl.enabled",
                         False)

spark_yarn_archive = default("configurations/spark2-defaults/spark.yarn.archive",
                             '/apps/spark/spark-yarn-archive.tar.gz')
...
# security settings
if security_enabled:
    spnego_principal = config['configurations']['spark2-defaults'][
        'history.server.spnego.kerberos.principal']
    spnego_principal = spnego_principal.replace('_HOST',
                                                socket.getfqdn().lower())
    spnego_keytab = config['configurations']['spark2-defaults'][
    ...
    if is_hive_installed:
        spark_hive_properties.update({
            'hive.metastore.sasl.enabled':
                str(config['configurations']['hive-site']
                    ['hive.metastore.sasl.enabled']).lower(),
        ...
        hive_kerberos_keytab = config['configurations'][
            'spark2-hive-site-override'][
            'hive.server2.authentication.kerberos.keytab']
        default_hive_kerberos_principal = config['configurations'][
            'spark2-hive-site-override'][
            'hive.server2.authentication.kerberos.principal']
        hive_kerberos_principal = default_hive_kerberos_principal.replace(
            '_HOST',
            socket.getfqdn().lower())

spark_transport_mode = config['configurations']['spark2-hive-site-override'][
...
if 'nm_hosts' in config['clusterHostInfo'] and len(
        config['clusterHostInfo']['nodemanager_hosts']) == 1:
...
hive_schematool_bin = format('{stack_root}/hive/bin')
...
livy_install_dir = stack_root + '/livy-server'
...
if "livy2-env" in config['configurations']:
    livy2_home = stack_root + '/livy-server'
    livy2_conf = '/etc/livy'
    ...
    livy2_recovery_dir = default(
        ...
        "/livy/recovery")
    livy2_recovery_store = default(
        "/configurations/livy2-conf/livy.server.recovery.state-store",
        "filesystem")
    ...
    livy2_log4j_properties = config['configurations'][
        'livy2-log4j-properties']['content']
    ...
    if 'livy.server.kerberos.principal' in config['configurations'][
        'livy2-conf']:
    ...
    livy2_http_scheme = 'https' if 'livy.keystore' in config['configurations'][
        'livy2-conf'] else 'http'
    ...
    livy2_livyserver_port = default(
        'configurations/livy2-conf/livy.server.port', 8999)
...
intel_daal_jar = stack_root + '/intel/daal/lib/daal.jar'
extra_class_path = '/usr/share/java/mysql-connector-java.jar'
if os.path.exists(intel_daal_jar):
    extra_class_path += ':' + intel_daal_jar

carbondata_content = config['configurations']['spark2-env'][
    'carbondata_content']

# region Atlas Hooks
hive_atlas_application_properties = default(
    '/configurations/hive-atlas-application.properties', {})
enable_atlas_hook = default('/configurations/spark2-env/spark.atlas.hook',
                            False)
atlas_hook_filename = default('/configurations/atlas-env/metadata_conf_file',
                              'atlas-application.properties')
atlas_application_content = default('/configurations/application-properties',
                                    {})
# endregion

# enable ranger
enable_ranger_spark = default(
    '/configurations/ranger-env/ranger-spark-plugin-enabled', 'no').lower()
