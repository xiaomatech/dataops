...
class LivyServer(Script):
    ...
    def configure(self, env):
        ...
        setup_livy(env, 'server', action='config')
    ...
    def start(self, env, upgrade_type=None):
        ...
        self.configure(env)
        livy2_service('server', action='start')

    def stop(self, env):
        import params
        env.set_params(params)

        livy2_service('server', action='stop')
        ...
        if not is_empty(dir_path):
            ...
            if WebHDFSUtil.is_webhdfs_available(params.is_webhdfs_enabled,
                ...
                dfs_ret_code = shell.call(
                    format(params.hadoop_bin_dir +
                           "/hdfs --config {hadoop_conf_dir} dfs -test -d " +
                           dir_path),
