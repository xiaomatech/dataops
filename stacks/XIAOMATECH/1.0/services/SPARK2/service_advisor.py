...
class Spark2ServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    """
        ...
        recommender.recommendSpark2ConfigurationsMetrics(configurations, clusterData, services, hosts)
    ...
    """
        if configurations and "spark2-defaults" in configurations and \
            ...
            return configurations["spark2-defaults"]["properties"][
                       "spark.history.kerberos.enabled"].lower() == "true"
            ...
            return services["configurations"]["spark2-defaults"]["properties"][
                       "spark.history.kerberos.enabled"].lower() == "true"
        else:
            return False
...
class Spark2Recommender(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def recommendSpark2ConfigurationsMetrics(self, configurations,
                                             clusterData, services, hosts):
        putSparkMetricsProperty = self.putProperty(configurations, "spark2-metrics-properties",
                                                   services)
        spark_metrics_content = '''
## Enable Graphite
...
*.source.jvm.class=org.apache.spark.metrics.source.JvmSource
        '''
        graphite_hosts = self.getComponentHostNames(services, "GRAPHITE", "GRAPHITE_API")
        if graphite_hosts is not None and len(graphite_hosts) > 0:
            graphite_hosts.sort()
            graphite_host = graphite_hosts[0]
            putSparkMetricsProperty('content', spark_metrics_content % graphite_host)
    ...
    """
        ...
        if configurations and "ranger-env" in configurations and \
                "ranger-spark-plugin-enabled" in configurations["ranger-env"]["properties"] and \
                configurations["ranger-env"]["properties"]['ranger-spark-plugin-enabled'].lower() == 'yes':
            putSparkProperty(
                "spark.sql.extensions",
                "org.apache.ranger.authorization.spark.authorizer.RangerSparkSQLExtension"
            )

        if configurations and "spark2-env" in configurations and \
                "spark.atlas.hook" in configurations["spark2-env"]["properties"] and \
                configurations["spark2-env"]["properties"]['spark.atlas.hook']:
            putSparkProperty(
    ...
    """
        ...
        if Spark2ServiceAdvisor.isKerberosEnabled(services, configurations):
            ...
            spark2_defaults = self.getServicesSiteProperties(
                services, "spark2-defaults")

            if spark2_defaults:
                putSpark2DafaultsProperty = self.putProperty(
                    configurations, "spark2-defaults", services)
                putSpark2DafaultsProperty('spark.acls.enable', 'true')
                putSpark2DafaultsProperty('spark.admin.acls', '')
                putSpark2DafaultsProperty('spark.history.ui.acls.enable',
                                          'true')
    ...
    """
        if Spark2ServiceAdvisor.isKerberosEnabled(services, configurations):
            zeppelin_site = self.getServicesSiteProperties(
                services, "zeppelin-site")

            if zeppelin_site and 'zeppelin.server.kerberos.principal' in zeppelin_site:
                zeppelin_principal = zeppelin_site[
                    'zeppelin.server.kerberos.principal']
                zeppelin_user = zeppelin_principal.split(
                    '@')[0] if zeppelin_principal else None

                if zeppelin_user:
                    livy2_conf = self.getServicesSiteProperties(
                        services, 'livy2-conf')

                    if livy2_conf:
                        superusers = livy2_conf[
                            'livy.superusers'] if livy2_conf and 'livy.superusers' in livy2_conf else None
                        ...
                        if zeppelin_user not in _superusers:
                            ...
                            putLivy2ConfProperty = self.putProperty(
                                configurations, 'livy2-conf', services)
                            putLivy2ConfProperty('livy.superusers',
                                                 ','.join(_superusers))
  ...
  """
    ...
    def validateSpark2ThriftSparkConfFromHDP25(
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "spark2-thrift-sparkconf")
