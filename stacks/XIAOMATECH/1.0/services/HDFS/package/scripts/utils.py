...
def safe_zkfc_op(action, env):
  ...
  """
    ...
    if action == "start":
        try:
            ZkfcSlave.status_static(env)
        except ComponentIsNotRunning:
            ZkfcSlave.start_static(env)
...
def service(action=None,
  ...
  """
    ...
    if name == "nfs3":
        pid_file = format(
            "{hadoop_pid_dir_prefix}/{params.root_user}/privileged-root-nfs3.pid"
        )
        custom_export = {
    ...
    if name == "nfs3":
        process_id_exists_command += " || " + as_sudo([
            "test", "-f", status_params.unprivileged_nfsgateway_pid_file
        ]) + " && " + as_sudo(
            ["pgrep", "-F", status_params.unprivileged_nfsgateway_pid_file])
    elif name == "datanode":
        process_id_exists_command = PidFiles(
            params.possible_datanode_pid_files).test_command()
    ...
    if name == "datanode" and (params.secure_dn_ports_are_in_use or PidFiles(
            [params.datanode_secure_pid_file]).is_process_running()):
        user = "root"
        if action == 'stop' and PidFiles([params.datanode_secure_pid_file
                                          ]).is_process_running():
            ...
            # a running instance
            hadoop_env_exports.update({
                'HDFS_DATANODE_SECURE_USER':
                    params.hdfs_user
            })

    hdfs_bin = format("{hadoop_bin_dir}/hdfs")
    ...
    if user == "root":
        ...
        cmd = format(
            "{ulimit_cmd} source {hadoop_conf_dir}/hadoop-env.sh ; {hdfs_bin} --config {hadoop_conf_dir} --daemon {action} {name}"
        )
        if options:
            cmd += " " + options
    ...
    if action == "start":
        ...
        if code != 0:
            code, out, err = shell.checked_call(("cat", pid_file),
                                                sudo=True,
                                                env=hadoop_env_exports,
...
def get_hdfs_binary(distro_component_name):
  ...
  """
    import params
    hdfs_binary = params.install_dir + '/bin/hdfs'
...
class PidFiles:
    ...
    def test_command(self):
        def test_pid_command(pid_file):
            return '(' + as_sudo(["test", "-f", pid_file]) + " && " + as_sudo(
                ["pgrep", "-F", pid_file]) + ')'
