...
def wait_for_safemode_off(hdfs_binary,
                          afterwait_sleep=0,
                          execute_kinit=False,
                          retries=115,
                          sleep_seconds=10):
...
def format_namenode(force=None):
    ...
    cluster_id_clause = format(
        "-clusterId '{dfs_ha_initial_cluster_id}'"
    ) if dfs_ha_initial_cluster_id else ''

    if not params.dfs_ha_enabled:
        if force:
            ExecuteHadoop(
                'namenode -format {cluster_id_clause}',
            ...
            if not is_namenode_formatted(params):
                Execute(
                    format(
                        ...
                        "/hdfs --config {hadoop_conf_dir} namenode -format {cluster_id_clause} -nonInteractive"
                    ),
                    user=params.hdfs_user,
                ...
                if not is_namenode_formatted(params):
                    try:
                        Execute(
                            format(
                                params.hadoop_bin_dir +
                                "/hdfs --config {hadoop_conf_dir} namenode -format -nonInteractive {cluster_id_clause}"
                            ),
                            user=params.hdfs_user,
                            path=[params.hadoop_bin_dir],
                            logoutput=True)
...
def bootstrap_standby_namenode(params, use_path=False, run_if_present=False):
    ...
    try:
        ...
        Logger.info("Boostrapping standby namenode: %s (run_if_present=%s)" %
                    (bootstrap_cmd, str(run_if_present)))
