    ...
    """
        ...
        for name_service in self.name_services:
            ...
            for nn_unique_id in self.nn_unique_ids:
                http_key = namenode_http_fragment.format(
                    name_service, nn_unique_id)
                https_key = namenode_https_fragment.format(
                    name_service, nn_unique_id)

                http_value = default("/configurations/hdfs-site/" + http_key,
                                     None)
                ...
                try:
                    ...
                    if not state:
                        ...
                        check_service_cmd = params.hadoop_bin_dir + "/hdfs haadmin -ns {dfs_ha_nameservices} -getServiceState {0}".format(
                            nn_unique_id)
                        code, out = shell.call(
    ...
    """
        ...
        if state in mapping:
            ...
            if hosts_in_state is not None and len(
                    hosts_in_state) == 1 and next(
                        iter(hosts_in_state)).lower() == host_name.lower():
                return True
        return False
