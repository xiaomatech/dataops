...
def prepare_upgrade_backup_namenode_dir():
  ...
  """
    ...
    failed_paths = []
    nn_name_dirs = params.dfs_name_dir.split(',')
    backup_destination_root_dir = params.namenode_backup_dir
    ...
    for nn_dir in nn_name_dirs:
        ...
        if os.path.isdir(namenode_current_image
                         ) and not os.path.isdir(backup_current_folder):
            try:
                os.makedirs(backup_current_folder)
                Execute(('cp', '-ar', namenode_current_image,
...
def reach_safemode_state(user, safemode_state, in_ha, hdfs_binary):
  ...
  """
    Logger.info(
        "Prepare to transition into safemode state %s" % safemode_state)
    import params
...
def prepare_rolling_upgrade(hdfs_binary):
  ...
  """
    ...
    if not params.upgrade_direction or params.upgrade_direction not in [
        ...
        raise Fail("Could not retrieve upgrade direction: %s" % str(
...
def finalize_upgrade(upgrade_type, hdfs_binary):
  ...
  """
    ...
    if summary is not None and summary.is_switch_bits:
        Logger.info(
            "The {0} switches the binaries only.  No need to call finalization."
