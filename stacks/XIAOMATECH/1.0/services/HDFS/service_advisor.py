...
class HDFSServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    """
        ...
        recommender.recommendHDFSConfigurationsTune(configurations, clusterData, services, hosts)
        recommender.recommendHDFSConfigurationsLDAP(configurations, clusterData, services, hosts)
...
class HDFSRecommender(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def recommendConfigurationsFromHDP206(self, configurations, clusterData,
                                          services, hosts):
        """
    ...
    """
        ...
        self.logger.info("Class: %s, Method: %s. Total Available Ram: %s" %
                         (self.__class__.__name__, inspect.stack()[0][3],
                          str(totalAvailableRam)))
        putHDFSProperty('namenode_heapsize',
                        max(int(totalAvailableRam / 2), 204800))
        putHDFSProperty = self.putProperty(configurations, "hadoop-env",
                                           services)
        putHDFSProperty('namenode_opt_newsize',
                        max(int(totalAvailableRam / 8), 128))
        putHDFSProperty = self.putProperty(configurations, "hadoop-env",
                                           services)
        putHDFSProperty('namenode_opt_maxnewsize',
                        max(int(totalAvailableRam / 8), 256))
        ...
        hdfs_mount_properties = [("dfs.datanode.data.dir", "DATANODE",
                                  "/data1/hadoop/hdfs/data", "multi"),
                                 ("dfs.namenode.name.dir", "DATANODE",
                                  "/data1/hadoop/hdfs/namenode", "multi"),
                                 ("dfs.namenode.checkpoint.dir",
                                  "SECONDARY_NAMENODE",
                                  "/data1/hadoop/hdfs/checkpoint", "multi")]
    ...
    def recommendConfigurationsFromHDP22(self, configurations, clusterData,
                                         services, hosts):
        """
    ...
    """
        ...
        putHdfsSitePropertyAttribute = self.putPropertyAttribute(
            configurations, "hdfs-site")
        putHdfsSiteProperty("dfs.datanode.max.transfer.threads",
        ...
        putHdfsSiteProperty("dfs.namenode.safemode.threshold-pct",
        ...
        putHdfsEnvProperty(
            'namenode_heapsize',
            max(int(clusterData['totalAvailableRam'] / 2), 204800))
        ...
        if datanodeHosts is not None and len(datanodeHosts) > 0:
            ...
            datanodeFilesM = len(
                datanodeHosts
            ) * dataDirsCount / 10  # in millions, # of files = # of disks * 100'000
            nn_memory_configs = [{
                'nn_heap': 40960,
                'nn_opt': 128
            }, {
...
class HDFSValidator(service_advisor.ServiceAdvisor):
  ...
  """
    ...
    def validateHDFSConfigurationsFromHDP22(self, properties,
                                            recommendedDefaults,
                                            configurations, services, hosts):
        """
    ...
    """
        ...
        try:
            wire_encryption_enabled = hdfs_site[
        ...
        VALID_TRANSFER_PROTECTION_VALUES = [
            TRANSFER_PROTECTION_AUTHENTICATION, TRANSFER_PROTECTION_INTEGRITY,
        ...
        if (not wire_encryption_enabled
                and  # If wire encryption is enabled at Hadoop, it disables all our checks
            ...
            try:
                data_transfer_protection_value = hdfs_site[
            ...
            if dfs_http_policy_value not in VALID_HTTP_POLICY_VALUES:
                validationItems.append({
                    ...
                    "item":
                        self.getWarnItem(
                            "Invalid property value: {0}. Valid values are {1}".
                                format(dfs_http_policy_value,
                                       VALID_HTTP_POLICY_VALUES))
                })
            ...
            if dfs_http_policy_value == HTTPS_ONLY:
                if not privileged_dfs_dn_port and data_transfer_protection_value is None:
                    important_properties = [
                        dfs_datanode_address, datanode_https_address
                    ]
                    ...
                    address_properties_with_warnings.extend(
                        important_properties)
                ...
                if any_nonprivileged_ports_are_in_use:
                    important_properties = [
                        dfs_datanode_address, datanode_http_address
                    ]
                ...
                if data_transfer_protection_value not in VALID_TRANSFER_PROTECTION_VALUES:
                    validationItems.append({
                        ...
                        "item":
                            self.getWarnItem(
                                "Invalid property value: {0}. Valid values are {1}."
                                    .format(data_transfer_protection_value,
                                            VALID_TRANSFER_PROTECTION_VALUES))
                    })
        return self.toConfigurationValidationProblems(validationItems,
                                                      "hdfs-site")
    ...
    def validateRangerAuthorizerFromHDP23(self, properties,
    ...
    """
        ...
        if ("RANGER" in servicesList) and (
                ranger_plugin_enabled.lower() == 'yes'):

            try:
                if hdfs_site['dfs.namenode.inode.attributes.provider.class'].lower(
