...
class RobotAdmin():
    ...
    def distribute_password(self, all_hosts=None, user="root"):
        ...
        for new_host in new_hosts:
            subprocess.call([
                ...
                '%s/.ssh/id_rsa' % home, self.password_file,
                '%s@%s:%s' % (user, new_host, dest)
            ])
    ...
    def client_install(self,
        ...
        if os.path.isfile(self.password_file):
            ...
            p1 = subprocess.Popen(['cat', self.password_file],
                                  stdout=subprocess.PIPE)
            p2 = subprocess.Popen(
                [
    ...
    def _all_hosts_remote(self, remote_machine):
        ...
        p = subprocess.Popen(
            ssh + [
                ...
                self.ambari_db_password_file + '`; ' + ' '.join(
                    self.psql_query[:-1]) + ' \\"' + self.psql_query[-1] +
                '\\";' + '"'
            ],
            stdout=subprocess.PIPE)
...
class FreeIPACommon(object):
        ...
        if not self.sudorule_exists(rulename):
            ...
            if "Users" in kwargs and len(kwargs["Users"]):
                subprocess.call([
                    ...
                    '--users=' + ','.join(kwargs["Users"]), rulename
                ])
    ...
    def user_exists(self, user):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(['ipa', 'user-show', user],
                                      stderr=devnull)
            return True
    ...
    def sudorule_exists(self, sudorule):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(['ipa', 'sudorule-show', sudorule],
                                      stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
    ...
    def service_exists(self, service):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(['ipa', 'service-show', service],
                                      stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
    ...
    def group_exists(self, group):
        try:
            with open(os.devnull, "w") as devnull:
                subprocess.check_call(['ipa', 'group-show', group],
                                      stderr=devnull)
            return True
        except (subprocess.CalledProcessError, IOError, OSError):
            pass
    ...
    def update_password(self, user, password, password_file=None):
        ...
        p2 = subprocess.Popen(['ipa', 'user-mod', user, '--password'],
                              stdin=p1.stdout)
        p1.stdout.close()
        p2.communicate()
...
class FreeIPA(FreeIPACommon):
    ...
    def __enter__(self):
        with open(os.devnull, "w") as devnull:
            p1 = subprocess.Popen(['cat', self.password_file],
                                  stdout=subprocess.PIPE)
            p2 = subprocess.Popen(['kinit', self.principal],
                                  stdin=p1.stdout,
                                  stdout=devnull)
            p1.stdout.close()
            p2.communicate()
