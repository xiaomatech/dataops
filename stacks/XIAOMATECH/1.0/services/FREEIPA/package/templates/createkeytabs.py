...
def yield_config(filename):
    ...
    with open(filename) as fp:
        ...
        for line in fp:
            if len(line.strip()):
                yield dict((n, v) for n, v in zip(topline,
                                                  line.strip().split(',')))
...
def check_keytabdict_consistency(keytabs):
    ...
    for keytab in keytabs:
        ...
        if not len(identity):
            raise NameError("I could not interpret the identity of " +
                            str(keytab))
        if not len(realm):
            raise NameError("I could not interpret the realm of " +
                            str(keytab))
    ...
    for keytab in keytabs:
        ...
        if intermediate_file not in already_created:
            ...
            if (identity in ["HTTP/" + popen("hostname -f")]
                ...
                popen("chown " + keytab["keytab file owner"] + ":" +
                      keytab["keytab file group"] + " " + intermediate_file)
        ...
        remote = Remote(keytab["host"])
        exists = remote.run("bash -c 'if [-d " +
                            os.path.dirname(keytab["keytab file path"]) +
                            " ]; then echo \"yes\"; fi; '")
        ...
        remote.run("chown " + keytab["keytab file owner"] + ":" +
                   keytab["keytab file group"] + " " +
                   keytab["keytab file path"])
        remote.run("chmod " + keytab["keytab file mode"] + " " +
                   keytab["keytab file path"])
...
if __name__ == "__main__":
    ...
    group_princ = [
        ...
        if keytab["principal type"] == "USER" and
        keytab["keytab file group"] != keytab["principal name"].split('@')[0]
    ]
    # Create user principals first
    ...
    if len(failed):
        ...
        raise RuntimeError("Failed to create some keytabs! " +
                           str(len(failed)) + "/" + str(len(keytabs)))
    else:
